...
try {
    ...
    $columns = [
        ...
        "p.platform",      // 7
        "net",             //8 (aggregated)
        "p.date",          //9 (ordered via $DATE_SAFE)
        "p.delivery_date", //10 (ordered via $DELIV_SAFE)
        "p.orderable_type", //11 (NEW - orderable_type)
        "p.delivery_pack_updated",       //12
        "p.Tracking",      //13
        "p.delivery_pack", //14
        "p.id"             //15
    ];
    ...
    $where[] = "p.status > 0";
    $where[] = "(p.orderable_type IS NULL OR p.orderable_type != 'receive_at_luckgems')";
    ...
    LEFT JOIN (
        SELECT root_id,
               ...
               SUM(is_packed) AS packed_boxes,
               SUM(needs_packing) AS total_boxes_to_pack
        FROM (
            ...
            SELECT
              ...
              END) AS has_tracking,
              COUNT(DISTINCT CASE 
                  WHEN o.delivery_pack = 1 
                   AND o.orderable_type != 'receive_at_luckgems'
                  THEN o.box_number 
                  ELSE NULL 
              END) AS is_packed,
              COUNT(DISTINCT CASE 
                  WHEN o.orderable_type != 'receive_at_luckgems'
                  THEN o.box_number 
                  ELSE NULL 
