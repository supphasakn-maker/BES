	fn.app.production_prepare.prepare.dialog_add = function() {
		$.ajax({
			url: "apps/production_prepare/view/dialog.prepare.add.php",
			type: "POST",
			dataType: "html",
			success: function(html){
				...
				$("form[name=form_addprepare] select[name=round_id]").unbind().change(function(){
					$.post("apps/production_prepare/xhr/action-load-round.php",{round_id:$(this).val()},function(response){
	...
	fn.app.production_prepare.prepare.add = function(){
		$.post("apps/production_prepare/xhr/action-add-prepare.php",$("form[name=form_addprepare]").serialize(),function(response){
			if(response.success){
	...
	$(".btn-area").append(fn.ui.button({
		...
		onclick : "fn.app.production_prepare.prepare.dialog_add()",
		caption : "Add"
	}));
