...
$("#tblOrder").DataTable({
	...
	"processing": true,
	"serverSide": true,
	"ajax": "apps/sales_bwd/store/store-order.php",	
	"aoColumns": [
	...
	"createdRow": function ( row, data, index ) {
		...
		if(data.delivery_id == null){
			$("td", row).eq(9).html(fn.ui.button("btn btn-xs btn-outline-danger","far fa-truck ","fn.app.sales_bwd.order.dialog_add_delivery("+data[0]+")"));
		}
		
		s = '';
		if(data.delivery_id != null){
			s += fn.ui.button("btn btn-xs btn-outline-warning mr-1","far fa-truck ","fn.app.sales_bwd.order.dialog_postpone("+data[0]+")");
			s += fn.ui.button("btn btn-xs btn-outline-danger mr-1","far fa-lock ","fn.app.sales_bwd.order.dialog_lock("+data[0]+")");
		}else{
			//s += fn.ui.button("btn btn-xs btn-outline-danger mr-1","far fa-unlock ","fn.app.sales.order.dialog_lock("+data[0]+")");
		}
	
		
		s += fn.ui.button("btn btn-xs btn-outline-dark mr-1","far fa-cut ","fn.app.sales_bwd.order.dialog_split("+data[0]+")");
		s += fn.ui.button("btn btn-xs btn-outline-dark mr-1","far fa-pen","fn.app.sales_bwd.order.dialog_edit("+data[0]+")");
		s += fn.ui.button("btn btn-xs btn-danger","far fa-trash","fn.app.sales_bwd.order.dialog_remove_each("+data[0]+")");
