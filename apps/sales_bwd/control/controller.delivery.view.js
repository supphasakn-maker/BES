...
$("#tblDelivery").DataTable({
	...
	"processing": true,
	"serverSide": true,
	"ajax": "apps/sales_bwd/store/store-delivery.php",
	"aoColumns": [
	...
	"createdRow": function ( row, data, index ) {
		...
		s += fn.ui.button("btn btn-xs btn-outline-dark","far fa-pen","fn.app.sales_bwd.delivery.dialog_edit("+data[0]+")");
		$("td", row).eq(8).html(s);
		
		s = '';
		s += fn.ui.button("btn btn-xs btn-outline-dark mr-2","far fa-dollar-sign","fn.app.sales_bwd.delivery.dialog_payment("+data[0]+")");
		s += fn.ui.button("btn btn-xs btn-outline-dark","far fa-file","fn.app.sales.sales_bwd.dialog_billing("+data[0]+")");
		$("td", row).eq(9).html(s);
