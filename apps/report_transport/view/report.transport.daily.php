  ...
  <div class="row">
    <div class="col-sm-6">
		...
		<tbody>
			...
			WHERE bs_deliveries_drivers.truck_type LIKE '%มอ%' AND bs_deliveries.delivery_date = '".$_POST['date']."'  AND bs_deliveries_drivers.emp_driver = '".$_POST['user_group_id']."' 
			AND  bs_orders.status > 0 GROUP BY delivery_date, fullname";
			$rst = $dbc->Query($sql);
		...
		<tbody>
			...
			LEFT JOIN bs_deliveries_drivers ON bs_deliveries.id = bs_deliveries_drivers.delivery_id
			LEFT JOIN bs_employees ON bs_deliveries_drivers.emp_driver = bs_employees.id
			WHERE bs_deliveries_drivers.truck_type LIKE '%รถ%' AND bs_deliveries.delivery_date = '".$_POST['date']."'  AND bs_deliveries_drivers.emp_driver = '".$_POST['user_group_id']."' 
			AND  bs_orders.status > 0 GROUP BY delivery_date, fullname";
			$rst = $dbc->Query($sql);
			while($order = $dbc->Fetch($rst)){
			?>
