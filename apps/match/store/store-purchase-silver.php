<?php
	...
	$columns = array(
		...
		"total_usd" => "FORMAT(bs_purchase_spot.amount*(bs_purchase_spot.rate_pmdc+bs_purchase_spot.rate_spot)*32.1507,2)",
	...
	$aaData = array();
	$data = $dbc->GetResult();
	$sql = "SELECT * FROM bs_mapping_usd_spots WHERE mapping_id IS NULL";
	$rst = $dbc->Query($sql);
	while($mapping = $dbc->Fetch($rst)){
		...
		$remain_amount = $mapping['amount']/(32.1507*($purchase['rate_spot']+$purchase['rate_pmdc']));
		
		array_push($aaData,array(
			"DT_RowId" => 'm_'.$mapping['id'],
			"id" => $purchase['id'],
			"supplier_id" => $purchase['supplier_id'],
			"supplier" => $supplier['name'],
			"type" => $purchase['type'],
			"amount" => number_format($remain_amount,4),
			"total_usd" => number_format($purchase['amount']*($purchase['rate_spot']+$purchase['rate_pmdc'])*32.1507,2),
	...
	$sql = "SELECT SUM(bs_purchase_spot.amount*(bs_purchase_spot.rate_pmdc+bs_purchase_spot.rate_spot)*32.1507) FROM bs_purchase_spot 
	LEFT JOIN bs_mapping_usd_spots ON bs_mapping_usd_spots.purchase_id = bs_purchase_spot.id 
	WHERE bs_mapping_usd_spots.id IS NULL 
	AND (bs_purchase_spot.type LIKE 'physical'
	OR bs_purchase_spot.type LIKE 'stock'
