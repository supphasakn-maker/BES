<div class="col-xl-12 mb-12">
	<table class="table table-striped table-sm table-bordered dt-responsive nowrap">
		...
		while($record = $dbc->Fetch($rst)){
			...
			$sql = "SELECT * FROM bs_purchase_usd WHERE date = '".$record['date']."' AND (bs_purchase_usd.type LIKE 'physical' OR bs_purchase_usd.type LIKE 'stock')  AND parent IS NULL AND status != 0";
			$rst_purchase = $dbc->Query($sql);
			while($item = $dbc->Fetch($rst_purchase)){
			...
			$balance_usd += $margin_usd;
				...
				echo '<tr>';
					...
					$margin_class = ($balance_usd > 0)?" text-success":" text-danger";
					echo '<td class="text-right pr-2'.$margin_class.'">'.number_format($balance_usd,2).'</td>';
