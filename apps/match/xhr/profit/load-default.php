<div class="col-xl-12 mb-12">
	<table class="table table-striped table-sm table-bordered dt-responsive nowrap">
		<thead>
			<tr>
				<th class="text-center table-dark" colspan="17">Date</th>
			</tr>
			<tr>
		...
		<tbody>
			...
			while ($record = $dbc->Fetch($rst)) {
				...
				try {
					$sql_trade = "SELECT 
						(
							(SELECT COALESCE(SUM(amount*rate_spot*32.1507), 0) FROM bs_sales_spot WHERE trade_id = bs_trade_spot.id) - 
							(SELECT COALESCE(SUM(amount*rate_spot*32.1507), 0) FROM bs_purchase_spot WHERE trade_id = bs_trade_spot.id)
						) as trade_diff
					FROM bs_trade_spot 
					WHERE bs_trade_spot.date = '" . $record['date'] . "'";
					$rst_trade = $dbc->Query($sql_trade);

					$trade_total = 0;
					while ($item_trade = $dbc->Fetch($rst_trade)) {
						$trade_total += $item_trade['trade_diff'];
					}

					// Get rate_exchange for the date
					$rate_exchange = 1; // default value
					$sql_rate = "SELECT rate_exchange FROM bs_purchase_usd 
								WHERE DATE(created) = '" . $record['date'] . "' 
								ORDER BY created DESC LIMIT 1";
					$rst_rate = $dbc->Query($sql_rate);
					if ($rate_item = $dbc->Fetch($rst_rate)) {
						$rate_exchange = $rate_item['rate_exchange'];
					} else {
						// If no rate for today, get the latest rate before today
						$sql_rate = "SELECT rate_exchange FROM bs_purchase_usd 
									WHERE DATE(created) < '" . $record['date'] . "' 
									ORDER BY created DESC LIMIT 1";
						$rst_rate = $dbc->Query($sql_rate);
					...
					$trade_value = $trade_total * $rate_exchange;
				} catch (Exception $e) {
					// If there's an error with the trade calculation, just set to 0
					$trade_value = 0;
				}
				...
				$margin_class = ($profit > 0) ? " text-success" : " text-danger";
				echo '<td class="text-right pr-2' . $margin_class . '">' . number_format($profit, 2) . '</td>';

				// Trade column
				$trade_class = ($trade_value > 0) ? " text-success" : " text-danger";
