...
fn.app.production_summarize.prepare.add_scrap = function () {
    ...
    $.post("apps/production_summarize/xhr/action-add-add_scrap.php",
        form.serialize(),
        function (response) {
...
fn.app.production_summarize.scrap.cancel = function (scrap_id) {
    ...
    fn.dialog.confirmbox("ยืนยันการยกเลิก", confirmMessage, function () {
        $.post("apps/production_summarize/xhr/action-cancel-scrap.php",
            { scrap_id: scrap_id },
            function (response) {
                if (response.success) {
...
$("#tblScrap").DataTable({
    ...
    "createdRow": function (row, data, index) {
        ...
        if (data.status == "0") {
            s += fn.ui.button("btn btn-xs btn-outline-danger", "far fa-trash", "fn.app.production_summarize.scrap.remove(" + data[0] + ")");
        } else {
            s += fn.ui.button("btn btn-xs btn-outline-warning", "fas fa-undo", "fn.app.production_summarize.scrap.cancel(" + data[0] + ")");
        }
