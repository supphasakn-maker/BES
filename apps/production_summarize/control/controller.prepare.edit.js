	...
	fn.app.production_summarize.prepare.append_import= function() {
		let s = '';
		
		let onChange = 'fn.app.production_summarize.prepare.calcuate_import()';
		s += '<tr>';
	...
	fn.app.production_summarize.prepare.calcuate_import = function() {
	...
	fn.app.production_summarize.prepare.dialog_add_pack = function(id) {
		$.ajax({
			url: "apps/production_summarize/view/prepare/dialog.pack.add.php",
			data: {id:id},
			type: "POST",
			dataType: "html",
	...
	fn.app.production_summarize.prepare.add_pack = function(id) {
		$.post("apps/production_summarize/xhr/action-add-pack.php",$("form[name=form_addpack]").serialize(),function(response){
			if(response.success){
	...
	fn.app.production_summarize.prepare.calculation = function(){
		$.post("apps/production_summarize/xhr/action-load-calcuate.php",{
	...
	fn.app.production_summarize.prepare.append_pack = function() {
		...
		s += '<tr>';
			s += '<td class="text-center"><a onclick="fn.app.production_summarize.prepare.remove_pack(this)" href="javascript:;" class="btn btn-sm btn-danger">X</a>';
	...
	fn.app.production_summarize.prepare.remove_pack = function(btn) {
	...
	fn.app.production_summarize.prepare.dialog_edit = function(id) {
		$.ajax({
			url: "apps/production_summarize/view/dialog.prepare.edit.php",
			data: {id:id},
			type: "POST",
			dataType: "html",
			success: function(html){
	...
	fn.app.production_summarize.prepare.edit = function(){
		$.post("apps/production_summarize/xhr/action-edit-prepare.php",$("form[name=form_editprepare]").serialize(),function(response){
			if(response.success){
	...
	fn.app.production_summarize.prepare.pack_update = function(input){

		
		$.ajax({
			url: "apps/production_summarize/xhr/action-pack-change.php",
			data: {
	...
	$("#tblPacking").DataTable({
		...
		"ajax": {
			...
			"url":"apps/production_summarize/store/store-packing.php"
		},
		"aoColumns": [
		...
		"createdRow": function ( row, data, index ) {
			...
			$("td", row).eq(5).html('<input type="number" step="0.0001" data-id="'+data.id+'" onchange="fn.app.production_summarize.prepare.pack_update(this)" class="form-control form-control-sm" value="'+(data.weight_actual==null?"":data.weight_actual)+'">');
			
			if(data.status=="0"){
				s += fn.ui.button("btn btn-xs btn-outline-danger mr-1","far fa-trash","fn.app.production_summarize.pack.remove("+data[0]+")");
			}else{
	...
	$("select[name=stock_prepare]").change(function(){
		$.post("apps/production_summarize/xhr/action-load-prepare.php",{id:$(this).val()},function(html){
			$("#OutputA").html(html);
		},"html");
	}).change();
	
	fn.app.production_summarize.prepare.calculation();
