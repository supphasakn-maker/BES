...
$product = $dbc->GetRecord("bs_products", "name", "id=" . $_POST['product']);
$subtitle = "ระหว่างวันที่ " . $_POST['date_from'] . " ถึงวันที่ " . $_POST['date_to'] . "  ประเภท " . $product['name'];
?>
...
<a class="btn btn-xl btn-outline-dark mr-1" href="#apps/schedule/index.php?view=printable_processing_product&date_from=<?php echo $_POST['date_from']; ?>&date_to=<?php echo $_POST['date_to']; ?>&product_id=<?php echo $_POST['product']; ?>" target="_blank"><i class="far fa-print"></i></a>
<table class="table table-sm table-bordered table-striped" id="tblDeposit">
    ...
    <tbody>
        ...
        $line = array(0, 0);
        $sql = "SELECT SUM(weight_expected) AS amount FROM bs_scrap_total WHERE product_id = '" . $_POST['product'] . "' AND DATE_FORMAT(submited,'%Y-%m') = '" . date('Y-m', $dateform) . "'";
        $rst = $dbc->Query($sql);
        ...
        $sql = "SELECT * FROM bs_productions WHERE product_id = '" . $_POST['product'] . "' AND PMR = 'BWS' AND id != 1741 AND submited IS NOT NULL AND submited BETWEEN '" . $_POST['date_from'] . "' AND '" . $_POST['date_to'] . "'";
        $rst = $dbc->Query($sql);
        $number = 1;
        while ($production = $dbc->Fetch($rst)) {
            $d1 = strtotime($production['created']);
            $d2 = date('Y-m-d', $d1);
            $sum = $dbc->GetRecord("bs_scrap_total", "SUM(weight_expected) AS amount", "DATE_FORMAT(submited,'%Y-%m') = '" . date('Y-m', $dateform) . "'AND product_id = '" . $_POST['product'] . "'");
        ...
        <div class="col-sm-4">
            <table class="table table-sm table-bordered table-striped">
                ...
                <tbody>
                    ...
                    WHERE bs_scrap_total.pack_name LIKE '%รอการผลิต%' AND bs_scrap_total.product_id = '" . $_POST['product'] . "'  AND DATE_FORMAT(bs_scrap_total.submited,'%Y-%m') = '" . date('Y-m', $dateto) . "'";
                    $rst = $dbc->Query($sql);
                    $number = 1;
        ...
        <div class="col-sm-4">
            <table class="table table-sm table-bordered table-striped">
                ...
                <tbody>
                    ...
                    FROM bs_scrap_total 
                    LEFT OUTER JOIN bs_productions ON bs_scrap_total.production_id = bs_productions.id 
                    WHERE bs_scrap_total.pack_name LIKE '%เม็ดเสียรอการ Refine%' AND bs_scrap_total.product_id = '" . $_POST['product'] . "' AND DATE_FORMAT(bs_scrap_total.submited,'%Y-%m') = '" . date('Y-m', $dateto) . "'";
                    $rst = $dbc->Query($sql);
                    $number = 1;
                    while ($scrap = $dbc->Fetch($rst)) {
