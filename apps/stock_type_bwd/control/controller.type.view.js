...
$("#tblType").DataTable({
	...
	"processing": true,
	"serverSide": true,
	"ajax": "apps/stock_type_bwd/store/store-type.php",	
	"aoColumns": [
	...
	"createdRow": function ( row, data, index ) {
		...
		$("td", row).eq(2).html(data.type=="1"?"รอรับคืน":"ตัดสต๊อค");
		s = '';
		s += fn.ui.button("btn btn-xs btn-outline-dark","far fa-pen","fn.app.stock_type_bwd.type.dialog_edit("+data[0]+")");
		$("td", row).eq(3).html(s);
	}
});
