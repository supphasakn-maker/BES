<?php
$total = $dbc->GetRecord("bs_orders", "SUM(amount)", "status > 0 AND DATE(date) = '" . date("Y-m-d", $today) . "'");
...
for ($t = $today - (86400 * 5); $t <= $today; $t += 86400) {
    $total_each = $dbc->GetRecord("bs_orders", "SUM(amount)", "status > 0 AND DATE(date) = '" . date("Y-m-d", $t) . "'");
    // แปลง null เป็น 0 สำหรับแต่ละวันด้วย
...
<div class="card h-100">
    <div class="card-body">
        <div class="flex-center justify-content-start mb-2">
            ...
            <span id="connections"><?php echo join(",", $data); ?></span>
        </div>
        <h6 class="text-primary">ยอดขาย (กิโลกรัม Bowins Silver)</h6>
        <p class="small text-secondary mb-0">ยอดขายประจำวันนี้ BWS</p>
    </div>
</div>
