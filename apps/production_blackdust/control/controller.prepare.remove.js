fn.app.production_blackdust.prepare.dialog_remove = function() {
    var item_selected = $("#tblPrepare").data("selected");
    $.ajax({
        url: "apps/production_blackdust/view/dialog.prepare.remove.php",
...
fn.app.production_blackdust.prepare.remove = function(id){
    bootbox.confirm("Are you sure to remove?", function(result){ 
        if(result){
            $.post("apps/production_blackdust/xhr/action-remove-prepare.php",{id:id},function(response){
