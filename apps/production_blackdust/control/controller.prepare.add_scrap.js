fn.app.production_blackdust.prepare.dialog_add_scrap = function(id) {
    $.ajax({
        url: "apps/production_blackdust/view/prepare/dialog.pack.scrap.php",
        data: {id:id},
        type: "POST",
        dataType: "html",
...
fn.app.production_blackdust.prepare.add_scrap= function(){
    $.post("apps/production_blackdust/xhr/action-add-add_scrap.php",$("form[name=form_add_scrap]").serialize(),function(response){
        if(response.success){
...
$("#tblScrap").DataTable({
    ...
    "ajax": {
        ...
        "url":"apps/production_blackdust/store/store-scrap.php"
    },
    "aoColumns": [
    ...
    "createdRow": function ( row, data, index ) {
        ...
        if(data.status=="0"){
            s += fn.ui.button("btn btn-xs btn-outline-danger mr-1","far fa-trash","fn.app.production_blackdust.scrap.remove("+data[0]+")");
        }else{
