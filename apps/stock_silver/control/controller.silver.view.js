...
$("#tblStockSilver").DataTable({
	...
	"processing": true,
	"serverSide": true,
	"ajax": "apps/stock_silver/store/store-silver.php",	
	"aoColumns": [
	...
	"createdRow": function ( row, data, index ) {
		...
		if ( $.inArray(data.DT_RowId, $("#tblStockSilver").data( "selected")) !== -1 ) {
		...
		s = '';
		s += fn.ui.button("btn btn-xs btn-outline-warning mr-1","far fa-trash","fn.app.stock_silver.silver.remove("+data[0]+")");
		$("td", row).eq(7).html(s);
	},
...
fn.ui.datatable.selectable("#tblStockSilver","chk_plan");