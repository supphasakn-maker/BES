	...
	fn.app.trust_receipt.tr.dialog_payment = function(id) {
		$.ajax({
			url: "apps/trust_receipt7days/view/dialog.tr.payment.php",
			data: {id:id},
			type: "POST",
			dataType: "html",
	...
	fn.app.trust_receipt.tr.payment = function(){
		$.post("apps/trust_receipt7days/xhr/action-add-payment.php",$("form[name=form_payment]").serialize(),function(response){
			if(response.success){
	...
	fn.app.trust_receipt.tr.dialog_payusd = function(id) {
		$.ajax({
			url: "apps/trust_receipt7days/view/dialog.tr.payusd.php",
			data: {id:id},
			type: "POST",
			dataType: "html",
			success: function(html){
	...
	fn.app.trust_receipt.tr.payusd = function(){
		$.post("apps/trust_receipt7days/xhr/action-add-payusd.php",$("form[name=form_payusd]").serialize(),function(response){
			if(response.success){
				fn.app.trust_receipt.tr.load();
