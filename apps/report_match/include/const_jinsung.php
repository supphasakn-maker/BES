...
$aContent = array(
    array(
     ...
     -
      ...
      +(SELECT COALESCE(SUM(CASE WHEN adj_supplier in (16,17,18,20,22,23,24,25,26) THEN 0 ELSE amount END ),0) FROM bs_purchase_spot WHERE date BETWEEN  '2025-01-01' AND '#sql_date2#' AND adj_supplier ='#supplier_id#' AND product_id = '#product_id#' AND noted ='Normal' AND parent IS NULL AND type='physical' )
      ...
      (SELECT COALESCE(SUM((rate_spot)*amount*32.1507),0) FROM bs_purchase_spot WHERE date BETWEEN '2025-01-01' AND '2025-01-01' AND supplier_id = '#supplier_id#' AND product_id = '#product_id#' AND status > 0  AND type != 'defer')
       ...
       +(SELECT COALESCE(SUM(CASE WHEN supplier_id in (14,16,17,18,19,20,21,11,22.24,25,26) THEN defer = '0' ELSE defer END ),0) FROM bs_defer_cost  WHERE date_defer BETWEEN  '2025-01-01' AND '#sql_date2#'  AND supplier_id = '#supplier_id#')",
    ),
    array(
        "supplier", //1
        "Purchase Physical",
        "SELECT (SELECT COALESCE(SUM(CASE WHEN adj_supplier in (16,17,18,20,22,23,24,25,26) THEN 0 ELSE amount END ),0) FROM bs_purchase_spot WHERE  adj_supplier ='#supplier_id#' AND product_id = '#product_id#' AND parent IS NULL AND type='physical' AND noted ='Normal' AND date = '#sql_date#')
