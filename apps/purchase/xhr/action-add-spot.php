...
try {
	...
	if ($dbc->Insert("bs_purchase_spot", $data)) {
		...
		if ($dbc->Insert("bs_purchase_spot_profit_bwd", $data)) {
			$bb_bwd = $dbc->GetID();
		} else {
			error_log("Failed to insert bs_purchase_spot_profit_bwd");
		}
		...
		$allowed_suppliers = ["17", "18", "22", "23", "24", "25", "26"];
		...
		if ($bb_bwd !== null) {
			$dbc->Update("bs_purchase_spot_profit_bwd", array("purchase" => $spot_id), "id=" . $bb_bwd);
		}
