<table class="table table-sm table-bordered table-striped">
	...
	<tbody>
		...
		where  DATE_FORMAT(bs_orders.date,'%Y-%m') = '" . $_POST['month'] . "' AND bs_orders.parent IS NULL  AND bs_orders.status > -1";
		$rst = $dbc->Query($sql);
...
<div class="mt-4">
	...
	<table class="table table-sm table-bordered table-striped">
		...
		<tbody>
			...
			LEFT OUTER JOIN bs_deliveries ON bs_orders.delivery_id = bs_deliveries.id  
			LEFT OUTER JOIN bs_employees ON bs_orders.sales = bs_employees.id
			where   DATE_FORMAT(bs_orders.date,'%Y-%m') = '" . $_POST['month'] . "' AND bs_orders.parent IS NULL  AND bs_orders.status > -1 AND bs_orders.usd > 0 ";
