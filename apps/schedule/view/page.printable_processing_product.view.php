<?php
$product = $dbc->GetRecord("bs_products", "name", "id=" . $_GET['product_id']);
...
<h1 class="text-center">รายงาน Processing Loss ตั้งแต่วันที่ <?php echo $_GET['date_from']; ?> ถึงวันที่ <?php echo $_GET['date_to']; ?>
    <br> ประเภท <?php echo $product['name']; ?>
</h1>
<br>
<table class="table table-sm table-bordered table-striped" id="tblDeposit">
    ...
    <tbody>
        <?php
        $date = date("Y-m-d", strtotime("+1 day", strtotime($_GET['date_from'])));
        $date_to = $_GET['date_to'];
        $date_form = $_GET['date_from'];
        $dateto   = strtotime($date_to);
        $dateform   = strtotime($date_form);
        date('Y-m', $dateform);
        date('Y-m', $dateto);
        $line = array(0, 0);
        $sql = "SELECT SUM(weight_expected) AS amount FROM bs_scrap_total WHERE  product_id = '" . $_GET['product_id'] . "'  AND DATE_FORMAT(submited,'%Y-%m') = '" . date('Y-m', $dateform) . "'";
        $rst = $dbc->Query($sql);
        ...
        $sql = "SELECT * FROM bs_productions WHERE product_id != 2  AND product_id != 5 AND product_id = '" . $_GET['product_id'] . "' AND id != 1741 AND PMR = 'BWS' AND submited IS NOT NULL AND submited BETWEEN '" . $_GET['date_from'] . "' AND '" . $_GET['date_to'] . "'";
        $rst = $dbc->Query($sql);
        $number = 1;
        while ($production = $dbc->Fetch($rst)) {
            $d1 = strtotime($production['created']);
            $d2 = date('Y-m-d', $d1);
            $sum = $dbc->GetRecord("bs_scrap_total", "SUM(weight_expected) AS amount", "DATE_FORMAT(submited,'%Y-%m') = '" . date('Y-m', $dateform) . "'AND product_id = '" . $_GET['product_id'] . "'");
            ...
            <table width="100%">
                <tr>
                    <td class="text-center">
                        ...
                        <h3 style="font-family: cursive;"> Natthawut Kritsadanon</h3>
                    </td>
                    <td class="text-center">
                        <div>________________________</div>
                        <h3 style="font-family: cursive;">Tussanee Sriavudh</h3>
                    </td>
                </tr>
            </table>
        </div>
    ...
    <div class="row">
        <div class="col-sm-4">
            <table class="table table-sm table-bordered table-striped">
                ...
                <tbody>
                    ...
                    WHERE bs_scrap_total.pack_name LIKE '%รอการผลิต%' AND bs_scrap_total.product_id = '" . $_GET['product_id'] . "' AND DATE_FORMAT(bs_scrap_total.submited,'%Y-%m') = '" . date('Y-m', $dateto) . "'";
                    $rst = $dbc->Query($sql);
                    $number = 1;
        ...
        <div class="col-sm-4">
            <table class="table table-sm table-bordered table-striped">
                ...
                FROM bs_scrap_total 
                LEFT OUTER JOIN bs_productions ON bs_scrap_total.production_id = bs_productions.id 
                WHERE bs_scrap_total.pack_name LIKE '%เม็ดเสียรอการ Refine%' AND bs_scrap_total.product_id = '" . $_GET['product_id'] . "' AND DATE_FORMAT(bs_scrap_total.submited,'%Y-%m') = '" . date('Y-m', $dateto) . "'";
                    $rst = $dbc->Query($sql);
                    $number = 1;
                    while ($scrap = $dbc->Fetch($rst)) {
