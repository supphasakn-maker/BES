...
$transfer = $dbc->GetRecord("bs_transfers", "*", "id=" . $_GET['tr_id']);
...
$columns = array(
	...
	"rate_exchange" => "FORMAT(bs_purchase_usd.rate_finance,4)",
	"thb" => "FORMAT(bs_purchase_usd.amount*bs_purchase_usd.rate_finance,2)",
...
$table = array(
	...
	"join" => array(
		array(
			"field" => "id",
			"table" => "bs_transfer_usd",
			"with" => "purchase_id"
		)
	),
	"where" => "bs_purchase_usd.bank LIKE '" . $transfer['bank'] . "' AND bs_transfer_usd.transfer_id IS NULL"
);
