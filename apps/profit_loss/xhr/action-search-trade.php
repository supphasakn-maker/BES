...
$order_condition = " bs_orders.parent IS NULL AND status > -1";
		...
		<table class="table table-striped table-sm table-bordered dt-responsive nowrap">
			...
			WHERE YEAR(date) > 2021 ";
				if ($_POST['month'] != "") {
					...
					$sql_order = "SELECT SUM(amount) FROM bs_orders WHERE YEAR(date) > 2021 AND date < '" . date("Y-m-d", $time_filter) . "' AND " . $order_condition;
					$rst = $dbc->Query($sql_order);
					$item = $dbc->Fetch($rst);
					$balance -= $item[0];


					$sql_purchase = "SELECT SUM(amount) FROM bs_purchase_spot WHERE YEAR(date) > 2021 AND date < '" . date("Y-m-d", $time_filter) . "' AND (bs_purchase_spot.type LIKE 'physical' OR bs_purchase_spot.type LIKE 'stock') AND parent IS NULL AND rate_spot > 0 AND status > -1";
				...
				while ($record = $dbc->Fetch($rst)) {
					...
					$purchase = array();
					$margin = 0;
					$sql = "SELECT COUNT(id),SUM(amount),SUM(total) FROM bs_orders WHERE DATE(date) = '" . $record['date'] . "' AND " . $order_condition;
				...
				) AND parent IS NULL AND rate_spot > 0 AND confirm IS NOT NULL
		...
		<table class="table table-striped table-sm table-bordered dt-responsive nowrap">
			...
			";
				...
				while ($record = $dbc->Fetch($rst)) {
					...
					$purchase = array();
					$margin = 0;
					$sql = "SELECT COUNT(id),SUM(amount),SUM(total) FROM bs_orders WHERE YEAR(date) = '" . $record[0] . "' AND MONTH(date) = '" . $record[1] . "' AND " . $order_condition;
					$rst_order = $dbc->Query($sql);
					while ($item = $dbc->Fetch($rst_order)) {
						array_push($order, array(
							$item[0],
				...
				OR bs_purchase_spot.type LIKE 'stock'
					...
					for ($i = 0; $i < $total_row; $i++) {
						...
						if ($i == 0) {
							...
							echo '<td class="text-center" rowspan="' . $total_row . '">' . number_format($balance, 4) . '</td>';
						}
						echo '</tr>';
					}
				}
