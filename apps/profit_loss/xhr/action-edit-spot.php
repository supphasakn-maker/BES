...
if (empty($_SESSION['auth']['user_id'])) {
    ...
    if ($dbc->Update("bs_purchase_spot_profit", $data, "id=" . $_POST['id'])) {
        echo json_encode(array(
            'success' => true
        ));

        $spot = $dbc->GetRecord("bs_purchase_spot_profit", "*", "id=" . $_POST['id']);
        $os->save_log(0, $_SESSION['auth']['user_id'], "spot-profit-edit", $_POST['id'], array("spot" => $spot));
    } else {
        echo json_encode(array(
            'success' => false,
            'msg' => "No Change"
