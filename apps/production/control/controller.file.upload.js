	...
	fn.app.production.produce.clear = function(){
		var id = $("form[name=form_uploader] input[name=id]").val();
		var path = $(id).find("input[xname=img_path]").val();
		$.post('apps/production/xhr/action-clear.php',{path:path},function(response){
			if(response.success){
				$(id).remove();
	...
	fn.app.production.produce.save = function(){
		var id = $("form[name=form_uploader] input[name=id]").val();
	...
	fn.app.production.produce.dialog_file = function(idx) {
		$.ajax({
			url: "apps/production/view/dialog.file.upload.php",
			data: {
	...
	$("form[name=form_uploader] input[type=file]").change(function(){
		...
		jQuery.ajax({
			url: 'apps/production/xhr/action-upload-file.php',
			data: data,
			cache: false,
			contentType: false,
			processData: false,
			...
			success: function(response){
				if(response.success){
					...
					for(i in response.uploaded){
						...
						s += '<a data-toggle="tooltip" data-placement="top" id="'+id+'" title="" onclick="fn.app.production.produce.dialog_file(\'#'+id+'\')" href="javascript:;" class="m-2">';
							s += '<input type="hidden" xname="img_path" name="img_path[]" value="'+path+'">';
							s += '<input type="hidden" xname="img_desc" name="img_desc[]" value="">';
							s += '<img style="height:100px;" class="img-thumbnail" src="'+path+'">';
						s += '</a>';
					}
					s += '';
					$("#img_frame").append(s);
