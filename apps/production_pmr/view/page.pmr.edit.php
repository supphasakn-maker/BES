...
<?php
	...
	$pmr = $dbc->GetRecord("bs_productions_pmr","*","id=".$_GET['id']);

    echo '<table id="tblPmrDetail" data-id="'.$pmr['id'].'" class="table table-bordered table-sm mt-2">';
		echo '<tr>';
			echo '<th class="text-center">id</th>';
			echo '<th class="text-center">รอบ</th>';
			echo '<th class="text-center">วันที่ส่งผลิต</th>';
		echo '</tr>';
		echo '<tr>';
		...
		echo '<div class="col-4">';
			echo '<div class="row">';
                ...
                echo '<label class="text-white">No</label>';
                    echo '<button onclick="fn.app.production_pmr.pmr.mapping()" class="btn btn-primary mr-2">Append</button>';
            echo '</div>';
			echo '</div>';
			...
			echo '<div class="row">';
				...
				echo '<div class="col-md-12">';
					echo '<button onclick="fn.app.production_pmr.pmr.toggle_check()" class="btn btn-warning mr-2">Toggle</button>';
					echo '<button onclick="fn.app.production_pmr.pmr.append_bulk()" class="btn btn-primary mr-2">Append</button>';
				echo '</div>';
			echo '</div>';
		echo '</div>';
...
?>
		...
		<ul class="list-group list-group-horizontal">
			<li class="list-group-item flex-fill text-center">
				<div class="text-secondary">Order Value</div><strong><?php echo number_format($pmr['weight_out_total'],4);?></strong>
			</li>
			<li class="list-group-item flex-fill text-center">
