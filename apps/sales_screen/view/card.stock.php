...
$aDate = array();
$PID = array(1, 3, 4, 5, 2, 6, 7, 8, 10);
$aBalance = array(1204, 206.6716, 0.009, 1.3003, 42, 0, 0, 0, 0);
$aTotal = array();
$aProduct = array();
...
for ($i = 0; $i < count($PID); $i++) {
	...
	$line_stockin = $dbc->GetRecord("bs_productions_in", "SUM(weight_out_total)", "submited BETWEEN '{$startDateStr}' AND '{$todayStr}' AND product_id = {$productId}");
	$stockin_amount = isset($line_stockin[0]) ? floatval($line_stockin[0]) : 0;
	$aBalance[$i] += $stockin_amount;
...
for ($i = 0; $i <= $totaldate; $i++) {
	...
	for ($j = 0; $j < count($PID); $j++) {
		...
		$line = $dbc->GetRecord("bs_productions_in", "SUM(weight_out_total)", "submited LIKE '{$currentDateStr}' AND product_id = {$productId}");
		$stockin_daily = isset($line[0]) ? floatval($line[0]) : 0;

		$total_in = $production_in + $stockin_daily;
		array_push($aaData_In[$j], $total_in);
		$aBalance[$j] += $total_in;
				...
				<tbody>
					...
					<tr>
						...
						foreach ($aDate as $date) {
							...
							echo '<td class="text-center text-dark font-weight-bold day-border" style="' . $style . '; --border-color: ' . $borderColor . ';" colspan="9">';
					...
					<!-- เข้า (รวม Production + Production In) -->
					<tr>
					...
					<!-- PMR (อื่นๆ) -->
					<tr>
						<td class="sticky-left text-center font-weight-bold" style="background-color: #dc3545; color: white;">PMR / ยืม</td>
						<?php
						for ($i = 0; $i < count($aDate); $i++) {
							$dayStyle = getDayColorStyleLight($aDate[$i]);
							$borderColor = getDayBorderColor($aDate[$i]);
					...
					<tr>
						...
						for ($i = 0; $i < count($aTotal); $i++) {
							...
							$borderColor = getDayBorderColor($aDate[$i]);
							echo '<td class="text-center day-border font-weight-bold" style="' . $dayStyle . '; --border-color: ' . $borderColor . ';" colspan="9">';
