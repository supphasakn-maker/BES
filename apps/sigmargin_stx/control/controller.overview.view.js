...
fn.app.sigmargin_stx.overview.calculate = function () {
	var s = '';
	...
	for (i in data) {
		...
		if (data[i][0] == 'NAG') {
			s += '<td class="text-center"><input type="text" class="form-control form-control-sm text-center" value="38.0000"></td>';
...
$('input[name=date]').change(function () {
	var date = $(this).val();
	console.log(date);
	$.post("apps/sigmargin_stx/xhr/overview/action-load-table.php",
		{ date: date },
		function (html) {
			$("#output").html(html);
			fn.app.sigmargin_stx.overview.calculate();
		}, "html");
});
