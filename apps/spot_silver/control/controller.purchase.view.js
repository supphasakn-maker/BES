...
$("#tblPurchase").DataTable({
	...
	"aoColumns": [
		...
		{ "bSortable": true, "data": "noted", "class": "text-center" },
		...
		{ "bSortable": true, "data": "currency", "class": "text-center" },
	...
	"createdRow": function (row, data, index) {
		...
		if (data.type == 'defer') {
			s = '';
			s += fn.ui.button("btn btn-xs btn-outline-warning mr-1", "far fa-trash", "fn.app.purchase.spot.remove(" + data[0] + ")");
			$("td", row).eq(12).html(s);
		} else {
			s = '';
