...
try {
    ...
    $columns = [
        ...
        "p.platform",                // 6
        "net",                       // 7 (aggregated)
        "p.date",                    // 8 (ordered via $DATE_SAFE)
        "p.delivery_date",           // 9 (ordered via $DELIV_SAFE)
        "p.orderable_type",          // 10
        "p.delivery_pack_updated",   // 11
        "p.Tracking",                // 12
        "p.delivery_pack",           // 13
        "p.accept_date",             // 14 
        "p.id"                       // 15
    ...
    $where[] = "p.status > 0";
    $where[] = "p.orderable_type = 'receive_at_luckgems'";
        ...
        FROM (
            ...
            SELECT
              ...
              COUNT(DISTINCT CASE 
                  ...
                  ELSE NULL 
              END) AS is_packed,
              COUNT(DISTINCT o.box_number) AS needs_packing
