...
(function () {
    ...
    function nextBusinessDay(d) {
        ...
        if (day === 5) { res.setDate(res.getDate() + 3); }
        else if (day === 6) { res.setDate(res.getDate() + 2); }
        else { res.setDate(res.getDate() + 1); }
        return res;
    }
  ...
  <div class="modal fade" id="deliveryDateModal" tabindex="-1" aria-hidden="true">
        ...
        <div class="modal-body pt-2">
          ...
          <div class="d-flex flex-column mb-3">
            ...
            <div class="form-group mb-2">
              <label for="ddm-custom-date" class="small mb-1">
                <i class="fa fa-calendar-alt mr-1"></i>หรือเลือกวันเอง:
              </label>
              <input type="date" class="form-control form-control-sm" id="ddm-custom-date">
            </div>
            <button type="button" class="btn btn-primary btn-block" id="btnSetCustomDate">
              <i class="fa fa-calendar-check mr-1"></i> ตั้งวันที่เลือก
            </button>
          </div>
    ...
    $(document).on('click', '#btnSetCustomDate', function () {
        const id = $('#ddm-order-id').val();
        const date = $('#ddm-custom-date').val();

        if (!date) {
            if (window.fn && fn.notify && fn.notify.warnbox) {
                fn.notify.warnbox('กรุณาเลือกวันที่', 'Warning');
            } else {
        ...
        if (id && date) {
            submitDeliveryDate(id, date);
        }
    });

    function ensureAcceptModal() {
        if ($('#acceptDateModal').length) return;
        const modalHtml = `
    <div class="modal fade" id="acceptDateModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-sm">
        <div class="modal-content">
            <div class="modal-header py-2 bg-success text-white">
            <h6 class="modal-title">บันทึกวันที่รับจริง</h6>
            <button type="button" class="close text-white" data-dismiss="modal" aria-label="Close"><span>&times;</span></button>
            </div>
            <div class="modal-body pt-2">
            <div class="mb-2">ออเดอร์: <b id="adm-order-code">-</b></div>
            
            <div class="form-group">
                <label for="acceptDateInput" class="font-weight-bold">
                <i class="fas fa-calendar-alt mr-1"></i>เลือกวันที่รับ:
                </label>
                <input type="date" 
                    class="form-control" 
                    id="acceptDateInput" 
            ...
            <button type="button" 
                    ...
                    id="btnConfirmAccept">
                <i class="fa fa-check-circle mr-1"></i>บันทึกวันที่รับ
            </button>
            
            <input type="hidden" id="adm-order-id" value="">
            </div>
        </div>
        </div>
    </div>`;
    ...
    function submitDeliveryDate(id, delivery_date) {
        $.ajax({
            url: "apps/prepare_pack_bwd_luckgems/xhr/create-delivery.php",
            method: "POST",
            dataType: "json",
    ...
    function submitAcceptDate(id, accept_date) {
        $.ajax({
            url: "apps/prepare_pack_bwd_luckgems/xhr/set-accept.php",
            method: "POST",
            dataType: "json",
            data: { id: id, accept_date: accept_date },
            success: function (resp) {
                if (resp && resp.success) {
                    if (window.fn && fn.notify && fn.notify.successbox) {
                        fn.notify.successbox(resp.msg || 'บันทึกวันที่รับเรียบร้อยแล้ว', 'Success');
                    } else {
                        alert(resp.msg || 'บันทึกวันที่รับเรียบร้อยแล้ว');
                    }
                    $('#acceptDateModal').modal('hide');
    ...
    const dt = $(TABLE_SEL).DataTable({
        ...
        ajax: {
            url: "apps/prepare_pack_bwd_luckgems/store/store-orders.php",
            method: "POST",
            type: "POST",
        ...
        aoColumns: [
            ...
            { bSortable: false, data: null, class: "text-center" },                     // 14 - ปุ่มรับจริง
            { bSort: true, data: "accept_date", class: "text-center" }                  // 15 - วันที่รับจริง
        ],
        order: [[10, "asc"]],
        ...
        createdRow: function (row, data) {
            try {
                ...
                // 14: ปุ่มรับจริง
                const isFullyPacked = packedBoxes >= totalBoxesToPack && totalBoxesToPack > 0;
                const acceptDate = data.accept_date || '';
                const accept = data.accept || 0;

                let acceptHtml = '';
                if (!isFullyPacked) {
                    acceptHtml = '<span class="badge badge-secondary" style="font-size:13px;">-</span>';
                } else if (accept == 1 && acceptDate) {
                    acceptHtml = `
                        <div class="d-flex align-items-center justify-content-center">
                            <span class="badge badge-success mr-2" style="font-size:13px; padding: 6px 12px;">
                                <i class="fas fa-check-circle mr-1"></i>รับแล้ว
                            </span>
                            <button type="button" 
                                    class="btn btn-sm btn-danger btn-cancel-accept" 
                                    data-id="${data.id}" 
                                    data-code="${data.code}"
                                    title="ยกเลิกการรับ">
                                <i class="fas fa-undo"></i>
                    ...
                    acceptHtml = `
                        <button type="button" 
                                class="btn btn-sm btn-primary btn-set-accept" 
                                data-id="${data.id}" 
                                data-code="${data.code}"
                                title="บันทึกวันที่รับ">
                            <i class="fas fa-calendar-check mr-1"></i>รับจริง
                        </button>
                    `;
                }
                $('td', row).eq(14).html(acceptHtml);

                // 15: วันที่รับจริง
                $('td', row).eq(15).html(acceptDate ? acceptDate : '<span class="text-muted">-</span>');
    ...
    function cancelPackStatus(id) {
        $.ajax({
            url: "apps/prepare_pack_bwd_luckgems/xhr/cancel-pack.php",
            method: "POST",
            dataType: "json",
            data: { id: id },
            success: function (resp) {
    ...
    // Click: บันทึกวันที่รับ
    // Click: บันทึกวันที่รับ
    $(document)
        .off('click.setaccept', '.btn-set-accept')
        .on('click.setaccept', '.btn-set-accept', function (e) {
            e.stopPropagation();
            const orderId = $(this).data('id');
            const orderCode = $(this).data('code');
            ...
            $('#adm-order-id').val(orderId);
            $('#adm-order-code').text(orderCode);
            $('#acceptDateInput').val(todayStr); // ตั้งค่าเริ่มต้นเป็นวันนี้

            $('#acceptDateModal').modal('show');
        });

    $(document).on('click', '#btnConfirmAccept', function () {
        const id = $('#adm-order-id').val();
        const date = $('#acceptDateInput').val();

        if (!date) {
            if (window.fn && fn.notify && fn.notify.warnbox) {
                fn.notify.warnbox('กรุณาเลือกวันที่รับ', 'Warning');
            } else {
        ...
        if (id && date) {
            submitAcceptDate(id, date);
        }
    });

    // Click: ยกเลิกการรับ
    $(document)
        .off('click.cancelaccept', '.btn-cancel-accept')
        .on('click.cancelaccept', '.btn-cancel-accept', function (e) {
            e.stopPropagation();
            const orderId = $(this).data('id');
            const orderCode = $(this).data('code');

            fn.dialog.confirmbox(
                `ต้องการยกเลิกการรับของออเดอร์ ${orderCode} ใช่หรือไม่?`,
                'ยืนยันการยกเลิก',
                function () {
                    cancelAcceptStatus(orderId);
                }
            );
        });

    function cancelAcceptStatus(id) {
        $.ajax({
            url: "apps/prepare_pack_bwd_luckgems/xhr/cancel-accept.php",
            method: "POST",
            dataType: "json",
            data: { id: id },
            success: function (resp) {
                if (resp && resp.success) {
                    if (window.fn && fn.notify && fn.notify.successbox) {
                        fn.notify.successbox(resp.msg || 'ยกเลิกการรับเรียบร้อยแล้ว', 'Success');
                    } else {
                        alert(resp.msg || 'ยกเลิกการรับเรียบร้อยแล้ว');
                    }
