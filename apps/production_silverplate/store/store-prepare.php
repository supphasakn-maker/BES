...
$columns = array(
	...
	"total_item_a" => "(SELECT COUNT(id) FROM bs_packing_items WHERE bs_packing_items.production_id = bs_productions.id AND pack_type LIKE 'ถุงปกติ')",
	"total_item_b" => "(SELECT COUNT(id) FROM bs_packing_items WHERE bs_packing_items.production_id = bs_productions.id AND pack_type LIKE 'ถุงกระสอบ')",
	"round_summary" => "bs_productions.round_summary",
	"product_id" => "bs_productions.product_id",
	"product_name" => "bs_products.name",
	"PMR" => "bs_productions.PMR",
);

$where = "1";

if (isset($_GET['date_from'])) {
	$where .= " AND date(bs_productions.created) BETWEEN '" . $_GET['date_from'] . "' AND '" . $_GET['date_to'] . "' AND round_summary = 0 AND product_id = '6' AND product_id = '8' AND product_id = '9'";
}
