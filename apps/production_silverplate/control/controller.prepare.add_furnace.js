fn.app.production_silverplate.prepare.dialog_add_furnace = function (id) {
    $.ajax({
        url: "apps/production_silverplate/view/prepare/dialog.pack.add_furnace.php",
        data: { id: id },
        type: "POST",
        dataType: "html",
...
fn.app.production_silverplate.prepare.add_furnace = function () {
    $.post("apps/production_silverplate/xhr/action-add-add_furnace.php", $("form[name=form_add_furnace]").serialize(), function (response) {
        if (response.success) {
...
$("#tblFurnace").DataTable({
    ...
    "ajax": {
        ...
        "url": "apps/production_silverplate/store/store-furnace.php"
    },
    "aoColumns": [
    ...
    "createdRow": function (row, data, index) {
        ...
        if (data.status == "0") {
            s += fn.ui.button("btn btn-xs btn-outline-danger mr-1", "far fa-trash", "fn.app.production_silverplate.furnace.remove(" + data[0] + ")");
        } else {
