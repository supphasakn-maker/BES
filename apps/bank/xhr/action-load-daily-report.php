	...
	<div class="card-body">
		<table class="table table-sm table-bordered">
			<tbody>
				...
				<?php
				$line = $dbc->GetRecord("bs_bank_statement", "SUM(amount)", "date = '" . $date . "' AND type = 2 AND bank_id = 7");
				$total -= $line[0];
				?>
				<tr>
				...
				<?php
				$line = $dbc->GetRecord("bs_bank_statement", "SUM(amount)", "date = '" . $date . "' AND type = 2 AND bank_id = 11");
				$total -= $line[0];
				?>
				<tr>
				...
				<?php
				$line = $dbc->GetRecord("bs_bank_statement", "SUM(amount)", "date = '" . $date . "' AND type = 2 AND bank_id = 10");
				$total -= $line[0];
				?>
				<tr>
				...
				<?php
				$line = $dbc->GetRecord("bs_bank_statement", "SUM(amount)", "date = '" . $date . "' AND type = 2 AND bank_id = 6");
				$total -= $line[0];
				?>
				<tr>
				...
				<?php
				$line = $dbc->GetRecord("bs_bank_statement", "SUM(amount)", "date = '" . $date . "' AND type = 2 AND bank_id = 15");
				$total -= $line[0];
				?>
				<tr>
				...
				<?php
				$line = $dbc->GetRecord("bs_bank_statement", "SUM(amount)", "date = '" . $date . "' AND type = 2 AND bank_id = 13");
				$total -= $line[0];
				?>
				<tr>
				...
				<?php
				$line = $dbc->GetRecord("bs_bank_statement", "SUM(amount)", "date = '" . $date . "' AND type = 2 AND bank_id = 14");
				$total -= $line[0];
				?>
				<tr>
				...
				<?php
				$line = $dbc->GetRecord("bs_bank_statement", "SUM(amount)", "date = '" . $date . "' AND type = 2 AND bank_id = 12");
				$total -= $line[0];
				?>
				<tr>
				...
				<?php
				$line = $dbc->GetRecord("bs_bank_statement", "SUM(amount)", "date = '" . $date . "' AND type = 2 AND bank_id = 16");
				$total -= $line[0];
				?>
				<tr>
				...
				<?php
				$line = $dbc->GetRecord("bs_bank_statement", "SUM(amount)", "date = '" . $date . "' AND type = 2 AND bank_id = 17");
				$total -= $line[0];
				?>
				<tr>
				...
				<?php
				$line = $dbc->GetRecord("bs_bank_statement", "SUM(amount)", "date = '" . $date . "' AND type = 2 AND bank_id = 18");
				$total -= $line[0];
				?>
				<tr>
				...
				foreach ($aTransfer as $tr) {
					$sql = "SELECT -SUM(TableA.amount) 
							...
							WHERE
								TableA.date = '" . $date . "' AND TableA.type = 1
				...
				while ($type = $dbc->Fetch($rst)) {
					...
					/*
						$line = $dbc->GetRecord(
							"bs_payment_items LEFT JOIN bs_payments ON bs_payments.id = bs_payment_items.payment_id",
							"SUM(bs_payment_items.amount),bs_payments.customer_id",
							"bs_payments.date_active = '".$date."' AND bs_payment_items.type_id = ".$type['id']);
						$total += $line[0];
						echo '<tr>';
							echo '<td>หัก '.$type['name'].'</td>';
							echo '<td>'.$customer['name'].'</td>';
							echo '<td class="text-right pr-2">'.number_format($line[0],2).'</td>';
						echo '</tr>';
						*/
				...
				?>
				<?php
				$sql = "SELECT * FROM bs_finance_static_values WHERE type = 2 AND start <= '$date' AND (end >= '$date' OR end IS NULL)";
				$rst = $dbc->Query($sql);
				while ($line = $dbc->Fetch($rst)) {
					echo '<tr>';
				...
				<tr>
					<td>ยอดรวมทั้งหมด</td>
					<td></td>
					<td class="text-right pr-2"><?php echo number_format($total, 2); ?></td>
				</tr>
				<?php
				...
				<tr>
					<td>Balance</td>
					<td></td>
					<td class="text-right pr-2"><?php echo number_format($total - $line[0], 2); ?></td>
				</tr>
