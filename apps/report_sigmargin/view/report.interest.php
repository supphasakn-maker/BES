<?php
include_once "../../sigmargin/include/engine.php";
...
<table id="tblJournal" class="table table-bordered table-sm">
	...
	<tbody>
		...
		$line = $dbc->GetRecord("bs_smg_rate", "*", "date < '" . date("Y-m-1", $date) . "' ORDER BY date DESC");
	...
	<thead>
		...
		<?php
		if ($dbc->HasRecord("bs_smg_cash", "DATE_FORMAT(date, '%Y-%m') = '" . date("Y-m", $date) . "'")) {
			$line = $dbc->GetRecord("bs_smg_cash", "amount", "DATE_FORMAT(date, '%Y-%m')='" . date("Y-m", $date) . "'");
			$cash = $line['amount'];
			echo '<tr>';
