<?php
	...
	$data = array(
		'#approved' => 'NOW()',
		'#approver_id' => $_SESSION['auth']['user_id'],
		"#solutions_user" => $os->auth['id'],
		'#status' => 2
	);
	...
	if($dbc->Update("bs_claims",$data,"id=".$_POST['id'])){
		...
		$os->save_log(0,$_SESSION['auth']['user_id'],"claim-approve",$_POST['id'],array("claim" => $claim));
		...
		$fields  =   'message=Approve การเคลม' ."\n".'วันที่: ' .$dd." / ".$time ."\n".
		'Code: ' .$claim['code'] ."\n".
