<?php
	...
	$bar_id = $_POST["bar_id"];
	$year = $_POST['year'];
	$customer_id = $dbc->GetRecord("bs_customer_groups","name","id=".$_POST['customer_id']);
?>
...
if ($_POST['customer_id'] == "12"){
		$sql = "SELECT
				...
				HAVING SUM(bs_orders.amount) >  '".$_POST['bar_id']."'
...
<?php
			...
			AND bs_orders.parent IS NULL 
			AND bs_orders.status > -1
			AND bs_customers.gid = ".$_POST['customer_id']."
			GROUP BY customer_id
			HAVING SUM(bs_orders.amount) >  '".$_POST['bar_id']."'
			ORDER BY SUM(bs_orders.amount) DESC";
	$rst = $dbc->Query($sql);
