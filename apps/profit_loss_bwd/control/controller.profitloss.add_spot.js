fn.app.profit_loss_bwd.profitloss.dialog_add_spot = function () {
    var date_filter = $('#tblSales_length input[name=date_filter]').val();
    $.ajax({
        url: "apps/profit_loss_bwd/view/dialog.spot.add.php",
        type: "POST",
        dataType: "html",
...
fn.app.profit_loss_bwd.profitloss.add_spot = function () {
    $.post("apps/profit_loss_bwd/xhr/action-add-spot.php", $("form[name=form_addspot]").serialize(), function (response) {
        if (response.success) {
...
$(".btn-area").append(fn.ui.button({
    ...
    onclick: "fn.app.profit_loss_bwd.profitloss.dialog_add_spot()",
    caption: "Add Spot"
}));