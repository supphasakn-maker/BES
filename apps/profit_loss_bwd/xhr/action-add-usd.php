...
if (empty($_SESSION['auth']['user_id'])) {
    ...
    if ($dbc->Insert("bs_purchase_usd_profit_bwd ", $data)) {
        $usd_id = $dbc->GetID();
        echo json_encode(array(
            'success' => true,
            'msg' => $usd_id
        ));
        $usd = $dbc->GetRecord("bs_purchase_usd_profit_bwd ", "*", "id=" . $usd_id);
        $os->save_log(0, $_SESSION['auth']['user_id'], "usd-profit-bwd-add", $usd_id, array("usd" => $usd));
    } else {
        echo json_encode(array(
            'success' => false,
            'msg' => "Insert Error"
