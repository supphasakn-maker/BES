<?php
    ...
    FROM bs_transfer_report WHERE bank = 'SCB' AND DATE_FORMAT(bs_transfer_report.date,'%Y-%m') = '".$_POST['month']."'";
    $rst = $dbc->Query($sql);
    ...
    SUM(load_balance) AS load_balance, SUM(balance) AS balance,SUM(tr_non_fix_usd) AS tr_non_fix_usd,
    SUM(interest_summary) AS interest_summary , SUM(interest_paid) AS interest_paid
    FROM bs_transfer_report WHERE bank = 'KBANK' AND DATE_FORMAT(bs_transfer_report.date,'%Y-%m') = '".$_POST['month']."'";
    $rst = $dbc->Query($sql);
    $transferscb = $dbc->Fetch($rst);
    ...
    SUM(load_balance) AS load_balance, SUM(balance) AS balance,SUM(tr_non_fix_usd) AS tr_non_fix_usd,
    SUM(interest_summary) AS interest_summary , SUM(interest_paid) AS interest_paid
    FROM bs_transfer_report WHERE bank = 'BBL' AND DATE_FORMAT(bs_transfer_report.date,'%Y-%m') = '".$_POST['month']."'";
    $rst = $dbc->Query($sql);
    $transferscb = $dbc->Fetch($rst);
