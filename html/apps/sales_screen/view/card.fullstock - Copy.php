<?php
	...
	$line_production = $dbc->GetRecord("bs_productions","SUM(weight_out_packing)","submited < '".date("Y-m-d",$today)."'");
	$line_order = $dbc->GetRecord("bs_orders","SUM(amount)","delivery_date < '".date("Y-m-d",$today)."' AND status > 0");
	
	//$balance = $line_production[0]-$line_order[0];
	$balance = 558;
	$balanceLB = 650;
	$balanceBar = 97;
	
	$line_production = $dbc->GetRecord("bs_productions","SUM(weight_out_packing)","submited BETWEEN '".$start_date."' AND '".date("Y-m-d",$today)." 00:00:00' AND product_id = 1");
	$line_order = $dbc->GetRecord("bs_orders","SUM(amount)","delivery_date BETWEEN '".$start_date."' AND '".date("Y-m-d",$today)."' AND status > 0 AND product_id = 1");
	$balance  += $line_production[0]-$line_order[0];
	
	$line_production = $dbc->GetRecord("bs_productions","SUM(weight_out_packing)","submited BETWEEN '".$start_date."' AND '".date("Y-m-d",$today)." 00:00:00' AND product_id = 3");
	$line_order = $dbc->GetRecord("bs_orders","SUM(amount)","delivery_date BETWEEN '".$start_date."' AND '".date("Y-m-d",$today)."' AND status > 0 AND product_id = 3");
	$balanceLB  += $line_production[0]-$line_order[0];
	
	$line_production = $dbc->GetRecord("bs_productions","SUM(weight_out_packing)","submited BETWEEN '".$start_date."' AND '".date("Y-m-d",$today)." 00:00:00' AND product_id = 2");
	$line_order = $dbc->GetRecord("bs_orders","SUM(amount)","delivery_date BETWEEN '".$start_date."' AND '".date("Y-m-d",$today)."' AND status > 0 AND product_id = 2");
	$balanceBar  += $line_production[0]-$line_order[0];
