fn.app.production_prepare.prepare.dialog_add_furnace = function(id) {
    $.ajax({
        url: "apps/production_prepare/view/prepare/dialog.pack.add_furnace.php",
        data: {id:id},
        type: "POST",
        dataType: "html",
...
fn.app.production_prepare.prepare.add_furnace = function(){
    $.post("apps/production_prepare/xhr/action-add-add_furnace.php",$("form[name=form_add_furnace]").serialize(),function(response){
        if(response.success){
...
$("#tblFurnace").DataTable({
    ...
    "ajax": {
        ...
        "url":"apps/production_prepare/store/store-furnace.php"
    },
    "aoColumns": [
    ...
    "createdRow": function ( row, data, index ) {
        ...
        if(data.status=="0"){
            s += fn.ui.button("btn btn-xs btn-outline-danger mr-1","far fa-trash","fn.app.production_prepare.furnace.remove("+data[0]+")");
        }else{
