...
<table id="tblOverview" class="table table-bordered table-sm">
    <thead class="thead-dark">
        ...
        <tr>
            <td colspan="5"></td>
            <td class="text-center"><input name="value_initial" class="form-control form-control-sm text-center" type="text" value="10%" onchange="fn.app.sigmargin_stx.overview.calculate()"></td>
            <td class="text-center"></td>
            <td class="text-center"><input name="value_variation" class="form-control form-control-sm text-center" type="text" value="7%" onchange="fn.app.sigmargin_stx.overview.calculate()"></td>
            <td class="text-center"></td>
            <td class="text-center"><input name="value_cut_loss" class="form-control form-control-sm text-center" type="text" value="5%" onchange="fn.app.sigmargin_stx.overview.calculate()"></td>
        </tr>
    </thead>
    <tbody>
    </tbody>
...
$sql = "
        SELECT * FROM `bs_smg_stx_payment` a INNER JOIN bs_smg_stx_receiving b ON a.date = b.date
...
if ($result = $dbc->query($sql)) {
    while ($row = $result->fetch_row()) {
        ...
        $rs_bs_smg_trade = $dbc->GetRecord("bs_smg_stx_trade", "*", "date='" . $row[1] . "' AND purchase_type = 'Buy' ORDER BY id DESC");
