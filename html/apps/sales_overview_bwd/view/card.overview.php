...
LEFT JOIN bs_orders_bwd parent ON d.id = parent.delivery_id
LEFT JOIN bs_orders_bwd o ON (
    o.id = parent.id OR o.parent = parent.id
)
WHERE DATE(d.delivery_date) LIKE '" . $date . "' 
      AND o.status > 0
    ";
$rst = $dbc->Query($sql);
...
$sql = "SELECT
		SUM(bs_orders_bwd.amount) AS amount,
		SUM(bs_orders_bwd.total) AS total,
		SUM(bs_orders_bwd.net) AS net,
		COUNT(bs_orders_bwd.id) AS total_order
	FROM bs_orders_bwd
	WHERE DATE(date) < '" . $date . "' AND DATE(date) >= '" . $start_date . "' AND bs_orders_bwd.status > 0;
...
WHERE DATE(d.delivery_date) < '" . $date . "' 
      AND DATE(d.delivery_date) >= '" . $start_date . "' 
      AND o.status > 0;";
$rst = $dbc->Query($sql);
...
<div class="row gutters-sm">
    <div class="col-xl-4">
        ...
        <button onclick="fn.dialog.open('apps/sales_overview_bwd/view/dialog.lock.lookup.php','#dialog_lock_lookup')" type="button" class="btn btn-warning">Lock Report</button>
    </div>
    <div class="col-xl-8">
        <table class="table table-striped table-sm table-bordered dt-responsive nowrap">
            <tbody>
                <tr>
                    ...
                    <td class="text-center"><?php echo number_format($balance['amount'], 2); ?></td>
                    <td class="text-center"><?php echo number_format($balance['net'], 2); ?></td>
                </tr>
                <tr>
                    ...
                    <td class="text-center"><?php echo number_format($total['net'], 2); ?></td>
                </tr>
                <tr>
                    ...
                    <td class="text-center"><?php echo number_format(-$delivery['net'], 2); ?></td>
                </tr>
                <tr>
