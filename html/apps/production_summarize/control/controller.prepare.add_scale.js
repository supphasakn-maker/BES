fn.app.production_summarize.prepare.dialog_add_scale = function(id) {
    $.ajax({
        url: "apps/production_summarize/view/prepare/dialog.pack.add_scale.php",
        data: {id:id},
        type: "POST",
        dataType: "html",
...
fn.app.production_summarize.prepare.add_scale= function(){
    $.post("apps/production_summarize/xhr/action-add-add_scale.php",$("form[name=form_add_scale]").serialize(),function(response){
        if(response.success){
...
$("#tblScale").DataTable({
    ...
    "ajax": {
        ...
        "url":"apps/production_summarize/store/store-scale.php"
    },
    "aoColumns": [
    ...
    "createdRow": function ( row, data, index ) {
        ...
        if(data.status=="0"){
            s += fn.ui.button("btn btn-xs btn-outline-danger mr-1","far fa-trash","fn.app.production_summarize.scale.remove("+data[0]+")");
        }else{
