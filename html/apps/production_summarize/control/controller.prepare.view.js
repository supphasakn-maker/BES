$("#tblPrepare").DataTable({
	...
	"serverSide": true,
	"ajax": {
		"url": "apps/production_summarize/store/store-prepare.php",
		"data": function ( d ) {
	...
	"createdRow": function ( row, data, index ) {
		...
		var s = '';
		if(data.status=="0"){
			s += fn.ui.button("btn btn-xs btn-outline-danger mr-1","far fa-trash","fn.app.production_summarize.prepare.remove("+data[0]+")");
			s += fn.ui.button("btn btn-xs btn-outline-warning mr-1","far fa-thumbs-up","fn.app.production_summarize.prepare.dialog_approve("+data[0]+")");
			s += fn.ui.button("btn btn-xs btn-outline-dark","far fa-pen","fn.navigate('production_summarize','view=prepare&section=edit&production_id="+data[0]+"')");
		}else{
			s += fn.ui.button("btn btn-xs btn-outline-dark mr-1","far fa-eye","fn.navigate('production_summarize','view=prepare&section=edit&production_id="+data[0]+"')");
