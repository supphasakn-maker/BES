<?php
	...
	$customer_group = $dbc->GetRecord("bs_customer_groups","name","id=".$_POST['customer_group_id']);
	$sale_group_id = $dbc->GetRecord("bs_employees","fullname","id=".$_POST['sale_group_id']);
	$bar_group_id = $_POST["bar_group_id"];
	$title = $aReportType[$_POST['type']];
	$year = $_POST['year'];
	$subtitle = "ยอดขาย กลุ่ม ".$customer_group['name']." Sales ".$sale_group_id['fullname']." ประจำปี ".$year;
?>
...
<table class="table table-sm table-bordered table-striped overflow-auto">
	...
	<?php
		...
		if ($_POST['customer_group_id'] == "12"){
			...
			AND bs_orders.sales = ".$_POST['sale_group_id']."
			GROUP BY customer_id
			HAVING SUM(bs_orders.amount) >  '".$_POST['bar_group_id']."'
...
<?php
			...
			AND bs_orders.status > -1
			AND bs_orders.sales = ".$_POST['sale_group_id']."
			AND bs_customers.gid = ".$_POST['customer_group_id']."
			GROUP BY customer_id
			HAVING SUM(bs_orders.amount) >  '".$_POST['bar_group_id']."'
			ORDER BY SUM(bs_orders.amount) DESC
		";
