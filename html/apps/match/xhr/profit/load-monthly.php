	...
	<table class="table table-striped table-sm table-bordered dt-responsive nowrap">
		...
		<tbody>
			...
			while ($record = $dbc->Fetch($rst)) {
				...
				WHERE YEAR(bs_mapping_silvers.mapped) = '" . $record[0] . "' AND MONTH(bs_mapping_silvers.mapped) = '" . $record[1] . "'";
				...
				WHERE YEAR(bs_mapping_usd.mapped) = '" . $record[0] . "' AND MONTH(bs_mapping_usd.mapped) = '" . $record[1] . "'";
				...
				// คำนวณ Trade Value สำหรับ Month/Year
				try {
					$sql_trade = "SELECT 
					...
					FROM bs_trade_spot 
					WHERE YEAR(bs_trade_spot.date) = '" . $record[0] . "' AND MONTH(bs_trade_spot.date) = '" . $record[1] . "'";
