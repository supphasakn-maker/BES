...
$("#tblOrder").DataTable({
    ...
    "serverSide": true,
    "ajax": {
        "url": "apps/tracking_bwd_2/store/store-order_tracking.php",
        "data": function (d) {
    ...
    "aoColumns": [
        ...
        { "bSort": true, "data": "delivery_pack" },
        { "bSort": true, "data": "delivery_pack_updated" },
        { "bSort": true, "data": "Tracking" },
        { "bSortable": false, "data": "id", "class": "text-center" }
    ],
    "order": [[8, "asc"]],
    "createdRow": function (row, data, index) {
        $("td", row).eq(0).html(fn.ui.button("btn btn-xs btn-outline-danger mr-1", "far fa-pen ", "fn.app.sales_screen_bwd_2.multiorder.dialog_edit_tracking(" + data[0] + ")"));

        if (data.delivery_pack == "0") {
            $("td", row).eq(10).html('<span class="badge badge-warning" style="font-size:16px;">ยังไม่ได้แพ็ค</span>');
        } else if (data.delivery_pack == "1") {
            $("td", row).eq(10).html('<span class="badge badge-primary" style="font-size:16px;">เตรียมแพ็คแล้ว</span>');
        }

        $("td", row).eq(1).html('<a href="#apps/schedule_bwd_2/index.php?view=printablemulti&order_id=' + data.id + '">' + data.code + '</a>');
    ...
    "drawCallback": function (settings) {
        fn.app.tracking_bwd_2.order.date_update();
    }
});



fn.app.tracking_bwd_2.order.date_update = function () {
    var s = '';
    s += '<table class="table table-xs mb-0">';
    s += '<tbody>';
    s += '<tr>';
    s += '<th width="20" class="p-0 m-0"><button onclick="fn.app.tracking_bwd_2.order.date_previous()" class="btn btn-xs btn-dark  m-0"><i data-feather="chevron-left"></i></button></th>';
    ...
    s += '<th width="20" class="p-0"><button onclick="fn.app.tracking_bwd_2.order.date_next()" class="btn btn-xs btn-dark m-0"><i data-feather="chevron-right"></i></button></th>';
    s += '</tr>';
    s += '<tbody>';
    s += '</table>';
...
fn.app.tracking_bwd_2.order.date_next = function () {
    date_base.setDate(date_base.getDate() + 1);
    fn.app.tracking_bwd_2.order.date_update();
};

fn.app.tracking_bwd_2.order.date_previous = function () {
    date_base.setDate(date_base.getDate() - 1);
    fn.app.tracking_bwd_2.order.date_update();
};