...
try {
    ...
    // ====== Build WHERE conditions (shared) ======
    $where = [];

    // บังคับโชว์เฉพาะที่มี delivery_date
    $where[] = "parent.delivery_date IS NOT NULL AND parent.delivery_pack != '0'";

    // กรอง platform ที่ต้องการ
    $where[] = "parent.platform IN('Exhibition','Facebook','IG','Website','LINE','SilverNow','WalkIN')";

    // กรองช่วงวันที่ (ถ้าระบุ) — ใช้ BETWEEN ตรง ๆ (ไม่มี OR)
    if (!empty($_GET['date_from'])) {
        $date_from = addslashes($_GET['date_from']);
        $date_to   = addslashes($_GET['date_to'] ?? $_GET['date_from']);
        // ถ้า delivery_date เก็บเป็น DATETIME และอยากเทียบแค่วันที่ ให้หุ้ม DATE()
        $where[] = "(DATE(parent.delivery_date) BETWEEN '$date_from' AND '$date_to')";
    }

    // กรองวันเดียวแบบตรง ๆ
    if (!empty($_GET['delivery_date'])) {
        $delivery_date = addslashes($_GET['delivery_date']);
        $where[] = "DATE(parent.delivery_date) = '$delivery_date'";
    }
