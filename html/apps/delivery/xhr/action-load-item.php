...
$sql = "
		...
		LEFT JOIN bs_delivery_pack_items ON bs_delivery_pack_items.item_id = bs_packing_items.id
		LEFT JOIN bs_pmr_pack_items ON bs_packing_items.id= bs_pmr_pack_items.item_id
		WHERE bs_packing_items.code LIKE '%" . $_GET['term'] . "%'
		AND bs_delivery_pack_items.id IS NULL AND bs_packing_items.status = 0 AND bs_pmr_pack_items.id IS NULL 
	";
