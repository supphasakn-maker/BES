...
if (empty($_SESSION['auth']['user_id'])) {
    ...
    if ($dbc->Insert("bs_spot_profit_daily", $data)) {
        $spot_id = $dbc->GetID();
        echo json_encode(array(
            'success' => true,
            'msg' => $spot_id
        ...
        $spot = $dbc->GetRecord("bs_spot_profit_daily", "*", "id=" . $spot_id);
        $os->save_log(0, $_SESSION['auth']['user_id'], "spot-daily-add", $spot_id, array("spot" => $spot));
    } else {
        echo json_encode(array(
            'success' => false,
            'msg' => "Insert Error"
