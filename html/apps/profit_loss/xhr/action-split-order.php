...
try {
    ...
    // แปลงเป็น float และกรองค่าที่มากกว่า 0
    $clean_amounts = array();
    ...
    // ตรวจสอบว่า order มีอยู่จริง ด้วย SQL โดยตรง
    ...
    error_log("Order found: " . ($order_row ? "Yes" : "No"));
    ...
    // แปลงจาก numeric array เป็น associative array (ใช้ตาม structure ที่กำหนด)
    $order = array(
    ...
    error_log("Exception in split order: " . $e->getMessage());
    ...
    error_log("Error in split order: " . $e->getMessage());
    error_log("Stack trace: " . $e->getTraceAsString());
