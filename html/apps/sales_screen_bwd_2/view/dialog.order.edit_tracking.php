...
class myModel extends imodal
{
    function body()
    {
        ...
        if (!empty($orders['customer_id'])) {
            $cus = $dbc->GetRecord("bs_customers_bwd", "*", "id=" . (int)$orders['customer_id']);
        }

        $customer_username = $cus ? $cus['username'] : ($orders['customer_name'] ?? 'ไม่พบข้อมูล');
        ...
        <form name="form_editorder">
            <div class="form-row">
                <div class="form-group col-md-4">
                    ...
                    <input type="text" class="form-control" readonly name="customer_name" placeholder="ชื่อลูกค้า" autocomplete="off" value="<?php echo $orders["customer_name"]; ?>">
                    <input type="hidden" name="id" class="form-control" value="<?php echo $orders['id']; ?>">
                </div>
                <div class="form-group col-md-4">
                    <label>Username</label>
                    <input type="text" class="form-control" readonly name="username" placeholder="username" autocomplete="off" value="<?php echo htmlspecialchars($customer_username); ?>">
                </div>
                <div class="form-group col-md-4">
                    ...
                    <select name="platform" class="form-control" disabled>
                        ...
                        <option <?php if ($orders['platform'] == 'Exhibition') echo "selected"; ?> value="Exhibition">Exhibition</option>
                    </select>
                </div>
                <div class="form-group col-md-4">
                    <label>เบอร์</label>
            ...
            <div class="form-row">
                ...
                <div class="form-group col-md-4">
                    ...
                    <input type="date" name="delivery_date" class="form-control" value="<?php echo $orders['delivery_date']; ?>" readonly>
                </div>
                <div class="form-group col-md-4">
                    <label>วิธีการส่งสินค้า</label>
                    <select name="shipping" id="shipping" class="form-control" disabled>
                        <?php
            ...
            <div class="form-group">
                ...
                <textarea class="form-control" name="shipping_address" rows="5" placeholder="ที่อยู่ในการจัดส่ง" readonly><?php echo $orders['shipping_address']; ?></textarea>
            </div>

            <div class="form-group">
                <label>ที่อยู่ออกใบเสร็จ</label>
                <textarea class="form-control" name="billing_address" rows="5" placeholder="ที่อยู่ในการออกใบเสร็จ" readonly><?php echo $orders['billing_address']; ?></textarea>
            </div>

            <div class="form-group">
                <label>หมายเหตุ</label>
                <textarea class="form-control" name="comment" rows="3" placeholder="หมายเหตุ" readonly><?php echo $orders['comment']; ?></textarea>
            </div>
            <div class="form-group">
                <label>หมายเลข Tracking</label>
                <input type="text" name="Tracking" class="form-control" placeholder="หมายเลข Tracking" autocomplete="off" value="<?php echo $orders['Tracking']; ?>">
            </div>
        </form>
...
$modal->setButton(array(
    ...
    array("action", "btn-danger", "Submit", "fn.app.sales_screen_bwd_2.multiorder.edit_tracking()")
));
$modal->EchoInterface();
