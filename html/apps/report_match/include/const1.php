<?php
  $aContent = array(
    ...
    array(
      "supplier",
      "Purchase Physical",
      "SELECT FORMAT(SUM(amount),4) FROM bs_purchase_spot WHERE date = '#sql_date#' AND supplier_id ='#supplier_id#' AND parent IS NULL AND type='physical' AND status > 0",
      "SELECT FORMAT(SUM(amount*(rate_spot+rate_pmdc)*32.1507),4) FROM bs_purchase_spot WHERE date = '#sql_date#' AND supplier_id ='#supplier_id#' AND parent IS NULL AND type='physical' AND status > 0"
    ), 
    ...
    "Hedge (short)", 
    "Different Discount", 
    "Total Pending Available", // เอาเป็นยอดยกมา 16
    ...
    array(
      "total",
      "Customer Pending",
      "SELECT REPLACE(JSON_EXTRACT(data, '$.customer_pending'),'\"','') FROM bs_match_data WHERE date = '#sql_date#' ",
      "SELECT '-'",
