...
$aContent = array(
  ...
  array(
    ...
    "SELECT +
     ...
     -
      ...
      +(SELECT COALESCE(SUM(CASE WHEN adj_supplier in (16,17,18,20,22,23,24) THEN amount = '0' ELSE amount END ),0) FROM bs_purchase_spot WHERE date BETWEEN  '2025-01-01' AND '#sql_date2#' AND adj_supplier ='#supplier_id#' AND noted ='Normal' AND parent IS NULL AND type='physical' )
    ...
    "SELECT 
      ...
      (SELECT COALESCE(-SUM(usd),0) FROM bs_suppliers_mapping WHERE id = '#supplier_id#' AND date = '2025-01-01')
       +(SELECT COALESCE(SUM(CASE WHEN adj_supplier in (14,16,17,18,19,20,21,11,22,23,24) THEN amount = '0' ELSE (rate_spot+rate_pmdc)*amount*32.1507  END ),0) FROM bs_purchase_spot WHERE  adj_supplier ='#supplier_id#' AND noted ='Normal' AND parent IS NULL AND type='physical' AND date BETWEEN  '2025-01-01' AND '#sql_date2#')
       ...
       +(SELECT COALESCE(SUM(CASE WHEN supplier_id in (14,16,17,18,19,20,21,11,22,23,24) THEN defer = '0' ELSE defer END ),0) FROM bs_defer_cost  WHERE date_defer BETWEEN  '2025-01-01' AND '#sql_date2#'  AND supplier_id = '#supplier_id#')
  ...
  array(
    "supplier", //1
    "Purchase Physical",
    "SELECT (SELECT COALESCE(SUM(CASE WHEN adj_supplier in (16,17,18,20,22,23,24) THEN amount = '0' ELSE amount END ),0) FROM bs_purchase_spot WHERE  adj_supplier ='#supplier_id#' AND parent IS NULL AND type='physical' AND noted ='Normal' AND date = '#sql_date#')
...
"
  ...
  array(
    "total",
    "เม็ดเงินที่หลอมวันนี้(จ้างหลอม) (PMR RECYCLE)", //21
  ...
  array(
    "total",
    "เม็ดเงินที่หลอมวันนี้(จ้างหลอม)-Shining gold", //22
  ...
  array(
    "total",
    "Silver Stock (SILVER ARTICLE)", //30
  ...
  array(
    "total",
    "Balance Position Trader ICBC", //41
    ...
    "SELECT 
        ...
        + (
            ...
            + (SELECT COALESCE(SUM(CASE 
                                    WHEN adj_supplier IN (16,17,18,20,22,23,24) THEN amount = '0' 
                                    ELSE amount 
                                  END), 0) 
               FROM bs_purchase_spot 
            ...
            + (SELECT COALESCE(SUM(CASE 
                                  ...
                                  END), 0)
               FROM bs_purchase_spot 
               WHERE adj_supplier IN (16,17,18,20,22,23,24) 
                 AND parent IS NULL 
            ...
            - (SELECT COALESCE(SUM(amount), 0) 
               ...
               WHERE import_date BETWEEN '2024-01-01' AND '#sql_date2#' 
                 AND supplier_id IN (1,6,14,16,17,18,19,20,21,11,22,23,24))
  ...
  array(
    "total",
    "Total Silver Balance (Thailand)", //45
    "SELECT 
        -- ========================================
        -- INITIAL BALANCE CALCULATION
        -- ========================================
      ...
      -- Product ID 8
        ...
        -- ========================================
        - (
            -- Daily orders vs deliveries
            (SELECT SUM(bs_orders.amount) AS amount
             FROM bs_orders
            ...
            + (SELECT SUM(bs_orders.amount) + 10482 AS amount 
               FROM bs_orders 
