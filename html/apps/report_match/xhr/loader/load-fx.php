...
$sql = "SELECT 
    ...
    +(SELECT COALESCE(SUM(CASE WHEN adj_supplier in (14,16,17,18,19,20,21,11,22,23,24) THEN amount = '0' ELSE (rate_spot+rate_pmdc)*amount*32.1507 END ),0) FROM bs_purchase_spot WHERE adj_supplier in (1,6) AND noted ='Normal' AND parent IS NULL AND type='physical' AND date BETWEEN '2025-01-01' AND '$date_for_sql')
    +(SELECT COALESCE(SUM(CASE WHEN adj_supplier in (1,6,14,19,21,11) THEN amount = '0' ELSE (rate_spot+rate_pmdc)*amount*32.1507 END ),0) FROM bs_purchase_spot WHERE adj_supplier in (16,17,18,20,22,23,24) AND currency ='USD' AND parent IS NULL AND (type='physical-adjust') AND noted ='Open-Adjust' AND date BETWEEN '2025-01-01' AND '$date_for_sql')
    +(SELECT COALESCE(-SUM(CASE WHEN supplier_id in (14,19,21,11) THEN value_usd_total = '0' ELSE value_usd_total END ),0) FROM bs_transfers WHERE date BETWEEN '2025-01-01' AND '$date_for_sql' AND supplier_id in (1,6,16,17,18,20,22,23,24) )
    +(SELECT COALESCE(SUM(CASE WHEN supplier_id in (14,19,21,11) THEN value_usd_deposit = '0' ELSE value_usd_deposit END ),0) FROM bs_transfers WHERE date BETWEEN '2025-01-01' AND '$date_for_sql' AND supplier_id in (1,16,17,18,20,22,23,24))
    +(SELECT COALESCE(-SUM(CASE WHEN supplier_id in (14,19,21,11) THEN value_adjust_trade = '0' ELSE value_adjust_trade END ),0) FROM bs_transfers WHERE date BETWEEN '2025-01-01' AND '$date_for_sql' AND supplier_id in (1,6,16,17,18,20,22,23,24))
    +(SELECT COALESCE(SUM(defer),0) FROM bs_defer_cost WHERE date_defer BETWEEN '2025-01-01' AND '$date_for_sql' AND supplier_id = 1)
    +(SELECT COALESCE(SUM(usd),0) FROM bs_match_deposit WHERE date BETWEEN '2025-01-01' AND '$date_for_sql' AND supplier_id in(1,6,18,22,23,24))
    -(SELECT COALESCE(-SUM(usd),0) FROM bs_match WHERE date BETWEEN '2025-01-01' AND '$date_for_sql' AND supplier_id in(1,18,22,23,24))
    -(SELECT COALESCE(SUM(usd),0) FROM bs_match_fx WHERE date BETWEEN '2025-01-01' AND '$date_for_sql' AND supplier_id in(6,18,22,23,24))
...
echo '</div>'; // End financial-dashboard div