...
class myModel extends imodal
{
	function body()
	{
		...
		if (count($items) < 2) {
			...
			echo '</div>';
			echo '<div class="form-group row">';
			echo '<label class="col-sm-2 col-form-label text-right">Products</label>';
			echo '<div class="col-sm-10">';
			echo '<select name="products" class="form-control">';
			echo '<option value="1">เม็ดเงิน (NON LBMA)</option>';
			echo '<option value="2">แท่งเงิน</option>';
			echo '<option value="3">เม็ดเงิน (LBMA)</option>';
			echo '<option value="4">เม็ดเงิน (RECYCLE)</option>';
			echo '</select>';
			echo '</div>';
			echo '</div>';
			echo '<div class="form-group row">';
...
$modal->setModel("dialog_combine_scrap", "Combine Scrap");
$modal->setExtraClass("modal-lg");
$modal->setButton(array(
	array("close", "btn-secondary", "Dismiss"),
	array("action", "btn-danger", "Combine", "fn.app.production_scrap.scrap.combine()")
));
$modal->EchoInterface();
