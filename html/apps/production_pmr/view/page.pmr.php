...
if ($this->GetSection() == "prepare") {
	include "view/page.pmr.edit.php";
} else {
	$today = time();
...
?>
	<div class="btn-area btn-group mb-2">
		<form name="filter" class="form-inline mr-2" onsubmit="return false;">
			...
			<button type="button" class="btn btn-primary" onclick='$("#tblPmr").DataTable().draw();'>Lookup</button>
		</form>
	</div>
	<table id="tblPmr" class="table table-striped table-bordered table-hover table-middle" width="100%">
		<thead class="bg-dark">
			<tr>
