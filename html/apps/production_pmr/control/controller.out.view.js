$("#tblOut").DataTable({
    ...
    "serverSide": true,
    "ajax": {
        "url": "apps/production_pmr/store/store_out.php",
        "data": function (d) {
    ...
    "createdRow": function (row, data, index) {
        ...
        var s = '';
        if (data.status == "0") {
            s += fn.ui.button("btn btn-xs btn-outline-danger mr-1", "far fa-trash", "fn.app.production_pmr.out.remove(" + data[0] + ")");
            s += fn.ui.button("btn btn-xs btn-outline-warning mr-1", "far fa-thumbs-up", "fn.app.production_pmr.out.dialog_approve(" + data[0] + ")");
        } else {
