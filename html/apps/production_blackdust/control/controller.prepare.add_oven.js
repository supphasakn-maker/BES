fn.app.production_blackdust.prepare.dialog_add_oven = function(id) {
    $.ajax({
        url: "apps/production_blackdust/view/prepare/dialog.pack.add_oven.php",
        data: {id:id},
        type: "POST",
        dataType: "html",
...
fn.app.production_blackdust.prepare.add_oven= function(){
    $.post("apps/production_blackdust/xhr/action-add-add_oven.php",$("form[name=form_add_oven]").serialize(),function(response){
        if(response.success){
...
$("#tblOven").DataTable({
    ...
    "ajax": {
        ...
        "url":"apps/production_blackdust/store/store-oven.php"
    },
    "aoColumns": [
    ...
    "createdRow": function ( row, data, index ) {
        ...
        if(data.status=="0"){
            s += fn.ui.button("btn btn-xs btn-outline-danger mr-1","far fa-trash","fn.app.production_blackdust.oven.remove("+data[0]+")");
        }else{
