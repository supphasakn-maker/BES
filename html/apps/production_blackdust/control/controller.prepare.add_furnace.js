fn.app.production_blackdust.prepare.dialog_add_furnace = function(id) {
    $.ajax({
        url: "apps/production_blackdust/view/prepare/dialog.pack.add_furnace.php",
        data: {id:id},
        type: "POST",
        dataType: "html",
...
fn.app.production_blackdust.prepare.add_furnace = function(){
    $.post("apps/production_blackdust/xhr/action-add-add_furnace.php",$("form[name=form_add_furnace]").serialize(),function(response){
        if(response.success){
...
$("#tblFurnace").DataTable({
    ...
    "ajax": {
        ...
        "url":"apps/production_blackdust/store/store-furnace.php"
    },
    "aoColumns": [
    ...
    "createdRow": function ( row, data, index ) {
        ...
        if(data.status=="0"){
            s += fn.ui.button("btn btn-xs btn-outline-danger mr-1","far fa-trash","fn.app.production_blackdust.furnace.remove("+data[0]+")");
        }else{
