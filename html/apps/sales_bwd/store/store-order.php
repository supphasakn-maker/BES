...
$columns = array(
	...
	"Tracking" => "bs_orders_bwd.Tracking"
);

$where = '';
if (isset($_GET['combine_mode'])) {
	$where .= " AND bs_orders_bwd.delivery_id IS NULL";
}

if (isset($_GET['date_from'])) {
	$where .= " AND ((bs_orders_bwd.delivery_date BETWEEN '" . $_GET['date_from'] . "' AND '" . $_GET['date_to'] . "')";
	$where .= " OR bs_orders_bwd.delivery_date IS NULL) ";
}


if (isset($_GET['delivery_date'])) {
	$where .= " AND bs_orders_bwd.delivery_date = '" . $_GET['delivery_date'] . "'";
}

if (isset($_GET['customer_id'])) {
	$where .= " AND bs_orders_bwd.customer_id = " . $_GET['customer_id'];
}
...
$table = array(
	...
	"where" => "bs_orders_bwd.status > 0" . $where
);
