	fn.app.import_combine.combine.dialog_split = function(id) {
		$.ajax({
			url: "apps/import_combine/view/dialog.combine.split.php",
			data: {id:id},
			type: "POST",
			dataType: "html",
	...
	fn.app.import_combine.combine.append_split = function() {
		var s ='';
	...
	fn.app.import_combine.combine.split = function(){
		var amount = parseFloat($("[name=form_splitimport] [name=amount]").val());
		var total = 0;
		...
		if(amount != total){
			...
			$.post("apps/import_combine/xhr/action-split-import.php",$("form[name=form_splitimport]").serialize(),function(response){
				if(response.success){
