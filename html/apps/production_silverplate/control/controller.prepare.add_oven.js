fn.app.production_silverplate.prepare.dialog_add_oven = function (id) {
    $.ajax({
        url: "apps/production_silverplate/view/prepare/dialog.pack.add_oven.php",
        data: { id: id },
        type: "POST",
        dataType: "html",
...
fn.app.production_silverplate.prepare.add_oven = function () {
    $.post("apps/production_silverplate/xhr/action-add-add_oven.php", $("form[name=form_add_oven]").serialize(), function (response) {
        if (response.success) {
...
$("#tblOven").DataTable({
    ...
    "ajax": {
        ...
        "url": "apps/production_silverplate/store/store-oven.php"
    },
    "aoColumns": [
    ...
    "createdRow": function (row, data, index) {
        ...
        if (data.status == "0") {
            s += fn.ui.button("btn btn-xs btn-outline-danger mr-1", "far fa-trash", "fn.app.production_silverplate.oven.remove(" + data[0] + ")");
        } else {
