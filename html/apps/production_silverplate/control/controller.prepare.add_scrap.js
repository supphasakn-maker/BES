...
fn.app.production_silverplate.prepare.add_scrap = function () {
    $.post("apps/production_silverplate/xhr/action-add-add_scrap.php", $("form[name=form_add_scrap]").serialize(), function (response) {
        if (response.success) {
...
$("#tblScrap").DataTable({
    ...
    "createdRow": function (row, data, index) {
        ...
        if (data.status == "0") {
            s += fn.ui.button("btn btn-xs btn-outline-danger mr-1", "far fa-trash", "fn.app.production_silverplate.scrap.remove(" + data[0] + ")");
        } else {
