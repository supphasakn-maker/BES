...
$aBalance = array(-37.0000, 97.0000, 48.0000, -12.0000, 27, 17.0000, 22.0000, -38.0000, -106.0000, -84.0000, 37.0000, 20.0000);
...
for ($i = 0; $i < count($PID); $i++) {
	...
	// เปลี่ยนจาก DATE(created) เป็น delivery_date
	$line_order = $dbc->GetRecord(
		"bs_orders_bwd",
		"SUM(amount)",
		"delivery_date BETWEEN '{$startDateStr}' AND '{$todayStr}' AND status > 0 AND product_type = {$productId} AND platform != 'LuckGems'"
	);
...
for ($i = 0; $i <= $totaldate; $i++) {
	...
	for ($j = 0; $j < count($PID); $j++) {
		...
		// เปลี่ยนจาก DATE(created) เป็น delivery_date สำหรับการสั่งซื้อ
		$line = $dbc->GetRecord("bs_orders_bwd", "SUM(amount)", "delivery_date = '{$currentDateStr}' AND status > 0 AND product_type = {$productId} AND platform != 'LuckGems'");
		...
		// เพิ่มข้อมูล delivery_date (เดิมอยู่แล้ว - ไม่เปลี่ยน)
...
if (!function_exists('getDayColorStyle')) {
...
if (!function_exists('getRowTypeStyle')) {
		...
		<div class="controls-section">
			...
			<div class="summary-cards mt-3" id="summaryCards">
				...
				<div class="summary-card">
					...
					<div class="label">ส่งออกรวม (Delivery)</div>
				</div>
			</div>
		</div>
					...
					<tbody>
						...
						<tr class="fade-in">
							<td class="row-label" style="background: white; color: #dc3545 !important; font-weight: bold;">
								<i class="fas fa-arrow-up me-2"></i>ส่งออก
							</td>
							<?php
							for ($i = 0; $i < count($aDate); $i++) {
								for ($j = 0; $j < count($PID); $j++) {
