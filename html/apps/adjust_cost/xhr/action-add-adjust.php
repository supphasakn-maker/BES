<?php
	...
	if($_POST['date']==""){
		...
		foreach($_POST['sales'] as $sales_id){
			...
			$total_sales += $sales_item['amount']*($sales_item['rate_spot']+$sales_item['rate_pmdc'])*$oz2kg;
			
			$value_sales += $sales_item['amount']*$sales_item['rate_spot']*$oz2kg;
			$discount_sales += $sales_item['amount']*$sales_item['rate_pmdc']*$oz2kg;
		}
		...
		foreach($_POST['purchase'] as $purchase_id){
			...
			$value_purchase += $purchase_item['amount']*$purchase_item['rate_spot']*$oz2kg;
			$discount_purchase += $purchase_item['amount']*$purchase_item['rate_pmdc']*$oz2kg;
		}
		...
		foreach($_POST['purchase_new'] as $purchase_id){
			...
			$total_purchase_new += $purchase_item['amount']*($purchase_item['rate_spot']+$purchase_item['rate_pmdc'])*$oz2kg;
			
			$value_purchase_new += $purchase_item['amount']*$purchase_item['rate_spot']*$oz2kg;
			$discount_purchase_new += $purchase_item['amount']*$purchase_item['rate_pmdc']*$oz2kg;
		}
		
		
		if(number_format($amount_sales,4) != number_format($amount_purchase,4)){
			$text = "Sales : " . $amount_sales . "\n";
			$text .= "Purchasae : " . $amount_sales . "\n";
			
			echo json_encode(array(
				'success'=>false,
				'msg' => $text."Amount is not match!"
			));
		}else if(number_format($amount_sales,4) != number_format($amount_purchase_new,4)){
			$text = "Sales : " . $amount_sales . "\n";
			$text .= "New Purchasae : " . $amount_purchase_new . "\n";
			echo json_encode(array(
				'success'=>false,
				'msg' => $text."Amount is not match!"
			));
			...
			$data = array(
				...
				"#value_adjust_cost" => $value_adjust_cost,
				"#value_adjust_discount" => $value_adjust_discount,
				"#value_net" => $value_net,  // Spot dif
				"#user" => $os->auth['id']
			);
