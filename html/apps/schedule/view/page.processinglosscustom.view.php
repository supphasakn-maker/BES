...
<h1 class="text-center">รายงาน Processing Loss ตั้งแต่วันที่ <?php echo $_GET['date_from']; ?> ถึงวันที่ <?php echo $_GET['date_to']; ?></h1>
<br>
<table class="table table-sm table-bordered table-striped" id="tblDeposit">
    ...
    <tbody>
        ...
        $sql = "SELECT * FROM bs_productions WHERE product_id != 2  AND product_id != 5 AND PMR = 'BWS' AND id != 1741 AND submited IS NOT NULL AND submited BETWEEN '" . $_GET['date_from'] . "' AND '" . $_GET['date_to'] . "'";
        $rst = $dbc->Query($sql);
        $number = 1;
