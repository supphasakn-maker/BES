<?php
	...
	$os = new oceanos($dbc);
		...
		foreach($_POST['sales'] as $sales_id){
			...
			$total_sales_spot_value += $sales_item['amount']*$sales_item['rate_spot'];
			$total_sales_spot_dicount += $sales_item['amount']*$sales_item['rate_pmdc'];
			$total_sales_spot_net += $sales_item['amount']*($sales_item['rate_spot']+$sales_item['rate_pmdc']);
		}
		...
		foreach($_POST['purchase'] as $purchase_id){
			...
			$total_purchase_spot_value += $purchase_item['amount']*$purchase_item['rate_spot'];
			$total_purchase_spot_dicount += $purchase_item['amount']*$purchase_item['rate_pmdc'];
			$total_purchase_spot_net += $purchase_item['amount']*($purchase_item['rate_spot']+$purchase_item['rate_pmdc']);
		}
		...
		foreach($_POST['purchase_new'] as $purchase_id){
			...
			$total_purchase_new_value += $purchase_item['amount']*$purchase_item['rate_spot'];
			$total_purchase_new_dicount += $purchase_item['amount']*$purchase_item['rate_pmdc'];
			$total_purchase_new_net += $purchase_item['amount']*($purchase_item['rate_spot']+$purchase_item['rate_pmdc']);
		}
