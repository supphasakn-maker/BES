...
class PriceMonitor
{
    ...
    private $db_user = 'silver_now';
    private $db_pass = '\aMc"v655AbF';
    ...
    // ------- Variable cache for os_variable lookups -------
    protected $var_cache = [];

    public function __construct()
    {
        // ให้เวลาตามไทย (ส่งผลกับวัน/ชั่วโมงทำการ)
    ...
    {
        // เปลี่ยนมาใช้ init + real_connect + connect timeout (ไม่กระทบผลลัพธ์)
        ...
        // ใช้ utf8mb4 (แสดงผลเหมือนเดิม แต่รองรับอักขระมากกว่า)
        if (!$this->connection->set_charset('utf8mb4')) {
            // ไม่ให้ตายทั้งโปรแกรม
    ...
    // --- เพิ่ม retry เบา ๆ + CONNECTTIMEOUT (ไม่เปลี่ยนสูตร/ผลลัพธ์ปกติ) ---
    protected function getGoldPrice()
    {
        ...
        while ($attempts-- > 0) {
            ...
            if ($http_code == 200 && $response) {
                ...
                if (isset($data['price'])) {
                    // คงพฤติกรรมเดิม: +0.8000
                    return $data['price'] + 0.8000;
                }
            }
                ...
                if (isset($data) && is_array($data)) {
                    foreach ($data as $rate) {
                        if (isset($rate['Family']) && $rate['Family'] === 'USD50') {
                            return isset($rate['Bill_DD_TT']) ? $rate['Bill_DD_TT'] : null;
                        }
                    }
                }
            }
    ...
    protected function calculateNewPrices($spot_price, $exchange_rate)
    {
        ...
        // --- NEW: change_buy from os_variable (signed delta, e.g., -700)
        $change_buy = $this->getVariable('change_buy', -700);

        // Buy follows Sell + change_buy (e.g. -700)
    ...
    protected function getPreviousDifference($new_buy, $new_sell, $current_prices = null)
    {
        ...
        if ($rs && mysqli_num_rows($rs) >= 2) {
            mysqli_fetch_assoc($rs); // ข้ามตัวแรก
...
<div class='status'>
    <strong>✅ ระบบพร้อมใช้งาน (เวอร์ชันสมบูรณ์)</strong><br>
    เวลาปัจจุบัน: " . date('Y-m-d H:i:s') . "<br>
    เกณฑ์การประกาศ: ราคาดิบใหม่ ห่างจากราคาประกาศก่อนหน้า ≥ 100 บาท
</div>
...
<div class='rounding-rule'>
    <strong>⚖️ กฎการปัดราคาใหม่</strong><br>
    <div class='feature-list'>
        <ul>
...
<h3>🧪 ทดสอบระบบ</h3>
<a href='?trigger=1' class='test-btn'>▶️ ทดสอบ Monitor</a>
<a href='price_script.php' class='test-btn'>🔧 Test Script</a>
...
<div class='update-info'>
    ...
    40,228.70 (ราคาดิบ) → 40,250 (ราคาประกาศ)<br><br>
    
    <strong>การเปรียบเทียบ:</strong><br>
    ราคาประกาศก่อนหน้า: 40,100 บาท<br>
    ราคาดิบใหม่: 40,228.70 บาท<br>
    ความต่าง: 128.70 บาท → <span style='color: #28a745; font-weight: 600;'>✅ ประกาศ</span> (≥ 100 บาท)
</div>

<h3>📱 ข้อมูลใน Telegram Message</h3>
<div class='feature-list'>
    <ul>
        <li>💰 ราคาประกาศ (ปัดแล้ว) - 40,250 / 39,950</li>
        <li>🔢 ราคาดิบ (ก่อนปัด) - 40,228.70 / 39,928.70</li>
        <li>📊 การเปรียบเทียบแบบละเอียด</li>
        <li>📈 ทิศทางการเปลี่ยนแปลง</li>
        <li>✅ เหตุผลที่ประกาศ</li>
        <li>📋 ข้อมูลเทคนิคครบถ้วน</li>
    </ul>
</div>

<h3>🚀 การติดตั้งใช้งาน</h3>
<div class='feature-list'>
    <ol>
        <li>บันทึกไฟล์นี้เป็น <strong>silver_monitor.php</strong></li>
        <li>อัปโหลดไปยัง Web Server</li>
        <li>ทดสอบด้วยการเข้า <strong>?trigger=1</strong></li>
        <li>ตั้ง Cron Job ให้รันทุก 5 นาที</li>
        <li>ติดตาม Telegram สำหรับการแจ้งเตือน</li>
    </ol>
</div>
...
<div class='feature-list'>
    <ul>
        <li>🔄 เปรียบเทียบราคาดิบใหม่กับราคาประกาศก่อนหน้า</li>
        <li>⚖️ กฎการปัดราคาที่ป้องกันการขาดทุน</li>
        <li>📊 เกณฑ์การประกาศที่ลดการรบกวนไม่จำเป็น</li>
        <li>📱 Telegram Message ที่แสดงข้อมูลครบถ้วน</li>
        <li>🕐 รองรับการทำงานในวันอาทิตย์ (ใช้ราคาเสาร์)</li>
        <li>🔒 การตรวจสอบวันหยุดนักขัตฤกษ์</li>
        <li>🌐 Web Interface สำหรับทดสอบและติดตาม</li>
        <li>🛠️ รองรับทั้ง Web Trigger และ CLI</li>
    </ul>
</div>
