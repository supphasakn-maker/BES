...
$("#tblSilver").DataTable({
	...
	{"bSortable":false			,"data":"id"		,"sClass":"hidden-xs text-center",	"sWidth": "20px"  },
		{"bSort":true			,"data":"mapped"	 ,class:"text-center"	},
		{"bSort":true			,"data":"amount"		,class:"text-right pr-2"	},
		{"bSort":true			,"data":"remark"		,class:"text-left"	},
		{"bSort":true			,"data":"order_amount"	,class:"text-right pr-2"	},
	...
	"createdRow": function ( row, data, index ) {
		...
		s += fn.ui.button("btn btn-xs btn-outline-dark","far fa-pen","fn.app.match.silver.dialog_remark("+data.id+")");
		$("td", row).eq(6).html(s);
	}
});
...
$("#tblSales").DataTable({
	...
	"createdRow": function ( row, data, index ) {
		...
		if(data.mapping_item_id != null){
			...
			$("td", row).eq(8).html(fn.ui.button("btn btn-xs btn-danger mr-1","far fa-trash","fn.app.match.silver.remove_order("+data.mapping_item_id+")"));
		}else{
			$("td", row).eq(8).html(fn.ui.button(
