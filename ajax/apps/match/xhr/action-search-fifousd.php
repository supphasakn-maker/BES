	...
	?>
		<div class="col-xl-12 mb-12">
			<table class="table table-striped table-sm table-bordered dt-responsive nowrap">
				...
				<body>
				<?php
				$sql = "SELECT date FROM ((SELECT DATE(date) AS date FROM bs_purchase_usd) UNION (SELECT date AS date FROM bs_purchase_spot)) AS tmp WHERE YEAR(date) > 2021 GROUP BY(date);";
				$rst = $dbc->Query($sql);
				$balance = -4710.36;
				while($record = $dbc->Fetch($rst)){
					$purchase_spot = array();
					...
					WHERE date = '".$record['date']."' AND parent IS NULL";
					$rst_purchase = $dbc->Query($sql);
					while($item = $dbc->Fetch($rst_purchase)){
						...
						//if(!$dbc->HasRecord("bs_purchase_usd","parent=".$item['id'])){
							array_push($purchase,array(
								...
								$item['amount'],
								$item['rate_exchange'],
								$item['rate_exchange']*$item['amount']
							));
							$margin += $item['amount'];
						//}
					}
