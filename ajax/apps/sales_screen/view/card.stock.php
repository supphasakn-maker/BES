...
<?php
	...
	for($i=0;$i<count($PID);$i++){
		...
		$line_deposit = $dbc->GetRecord(
		"bs_stock_adjusted LEFT JOIN bs_stock_adjuest_types ON bs_stock_adjusted.type=bs_stock_adjuest_types.id",
		"SUM(bs_stock_adjusted.amount)",
		"bs_stock_adjusted.date < '".date("Y-m-d",$today)."'
		AND bs_stock_adjuest_types.type = 1
		AND bs_stock_adjusted.product_id = ".$PID[$i]);
		
		$aBalance[$i]  += $line_production[0]-$line_order[0]+$line_deposit[0];
	}
	...
	for($i=0;$i<=$totaldate;$i++){
		...
		for($j=0;$j<count($PID);$j++){
			...
			$line = $dbc->GetRecord("bs_orders","SUM(amount)","DATE(delivery_date) LIKE '".date("Y-m-d",$iDate)."' AND status > 0 AND product_id = ".$PID[$j]);
...
<a href="javascript:;" class="" onclick="$('.hidebyclick').toggle()">Toggle Detail</a>
	...
	<div class="card-body bg-dark overflow-auto">
		<table class="table table-stock table-bordered table-sm text-white ">
			<tbody>
				<tr>
					<th align="center" style="min-width:120px"><input type="hidden" name="today" value="2020-11-25"></td>
					<?php
